...
CREATE FUNCTION [dbo].[fn_determine_local_call] (@fromNPANXX char(6), @toNPANXX char(6))  
RETURNS int  AS  
BEGIN 
	declare @fromNPA char(3) , @toNPA char(3) , @i  int ;
	...
	If  (dbo.fn_CalculateMileage (@fromNPANXX, @toNPANXX  ) < 2)  
		return 1 ;	 
	if ( @fromNPA = '951' and @toNPA ='951')
		return 1;
	if ( @fromNPA = '910' and @toNPA ='910')
		return 1;
    if(select count(*) from tblLocal  with(nolock) where  frNPANXX = @fromNPANXX  and  NPA = @toNPA  and  (NXX =  right(@toNPANXX,3) or NXX='*')   and [Local] = 'Y')  > 0 
	 Begin
		Return 1;
	 End
	set @i =0 ;
	
	If( @fromNPA  in ( '402' , '757',  '218' ,   '559' ,'770' , '605') or @fromNPANXX in( '310802','310750','310349','310412' ))
	 Begin
