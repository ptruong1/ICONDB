...
CREATE PROCEDURE [dbo].[p_insert_billed_calls4]
@callDate	varchar(6),
...
If(@billtype =  '06')
 ...
 begin
	...
	If(@billtype = '07'  and @duration >2 ) 
	 Begin
		
		SET @callRevenue = dbo.fn_CalculateCallRevenue(@firstMinute  ,@addMinute ,@connectFee ,@duration ,@callType  ,@PIP  ,@minDuration ) 
		Update tbldebit SET balance = balance - @CallRevenue, modifydate = getdate(),userName='Enduser',[status]=1 where AccountNo = @creditCardNo	
	...
	if(@duration >2)
	  Begin
		 INSERT INTO  tblCallsbilled( RecordID,Calldate, ConnectTime, FromNo ,   ToNo, BillToNo ,  MethodOfRecord,billType,callType, FromState, FromCity ,  
			...
			AgentID, RecordFile, MinIncrement, channel, folderDate, InmateID,PIN,facilityID, InRecordFile	,duration, CallRevenue, PhoneType)
		 Values(@recordID,@Calldate, @ConnectTime, @FromNo ,@ToNo, @BillToNo ,  @MethodOfRecord,@billType,@callType, @FromState, @FromCity , 
			...
			@UserName,@errorCode, @RateClass, @firstMinute,@addMinute,@connectFee,@Pip,@MinDuration,@recordDate,@ratePlanID,@DbError, @ResponseCode,
			 @AgentID,@RecordFileName,@increment, @channel,@folderDate	,@inmateID,@PIN, @facilityID, @InRecordFile,@duration, @CallRevenue ,@phoneType)

	  End
		
		
	If(@billtype = '10 '  AND   @duration >2)  -- Update balance
	  Begin
		If(left(@toNo,3) <>  '011')
		 Begin
			Update tblPrepaid SET balance = balance -  @CallRevenue, modifydate = getdate() , UserName ='Enduser', status=1 where PhoneNo =@toNo	
