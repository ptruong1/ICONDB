...
-- =============================================
CREATE PROCEDURE [dbo].[p_get_visit_videos_By_Agent] 
  ...
  On A.facilityID = B.FacilityID and A.InmateID = B.InmateID
	...
	Left Join  [leg_Icon].[dbo].tblVisitStatus Y
	    On A.status = Y.StatusID
			...
			Where
			A.FacilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1)
  ...
  left Join  [leg_Icon].[dbo].tblVisitors D
	...
	Left Join  [leg_Icon].[dbo].tblVisitStatus Y
	    On A.status = Y.StatusID
			...
			Where
			A.FacilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1) and 
...
, A.StationID, L.LocationName
  ...
  left Join  [leg_Icon].[dbo].tblVisitors D
	...
	Select A.facilityID, RoomID as recordID, [ApmDate]   ,[ApmTime], A.LimitTime, A.VisitorID, D.Email, A.EndUserID, A.Status, A.InmateID, A.InmateName, D.[VLastName], D.[VFirstName],
  B.PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	   A.VisitType, X.Descript, InmateLogInID, VisitorLogInID, Note, isNull(AlertCellPhone,'') as AlertCellPhone,  
	  isNull(AlertCellCarrier,'') as AlertCellCarrier, Y.Descript as ApptStatus, D.Phone1, ISNULL(TotalCharge,0) as TotalCharge, A.RecordOpt, ISNULL(A.RecordStatus,0) as RecordStatus, C.ChatServerIP as ChatServerIP
, A.StationID, L.LocationName
  ...
  left Join  [leg_Icon].[dbo].tblVisitors D
	...
	Left Join  [leg_Icon].[dbo].tblVisitStatus Y
	    On A.status = Y.StatusID
			...
			order by A.ApmDate desc, A.ApmTime Desc
	Else
  If ( @VisitType <> -1 ) and (@VisitStatus <> -1) and (@VisitorFName <> '') Select A.facilityID, RoomID as recordID, [ApmDate]   ,LEFT(ApmTime,8) as ApmTime, A.LimitTime, A.VisitorID, D.Email, A.EndUserID, A.Status, A.InmateID, A.InmateName, D.[VLastName], D.[VFirstName],
  B.PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	   A.VisitType, X.Descript, InmateLogInID, VisitorLogInID, Note, isNull(AlertCellPhone,'') as AlertCellPhone,  
	  isNull(AlertCellCarrier,'') as AlertCellCarrier, Y.Descript as ApptStatus, D.Phone1, ISNULL(TotalCharge,0) as TotalCharge, A.RecordOpt, ISNULL(A.RecordStatus,0) as RecordStatus, C.ChatServerIP as ChatServerIP
