...
CREATE PROCEDURE [dbo].[p_determine_Facility4]
@ANI	varchar(10)  ,
...
@DID		varchar(10) OUTPUT,
@UseFor	tinyint  OUTPUT 
...
Declare  @NPA varchar(3), @ANIdayRestrict tinyint, @ANITimeRestrict tinyint, @FacDayRestrict  tinyint, @FacTimeRestrict  tinyint , @localtime datetime, @day int, @h int, @DivisionID int, @locationID int,
	...
	@DivisionTimeRestrict  bit,  @locationTimeRestrict bit ,  @nxx char(3), @timeZone smallint
SET @facilityID = 0
SET @IDrequired =0
SET   @PINRequird  =0
SET @AgentID =0
...
IF(@LocationID >0)
	SELECT  @LocationPINReq =ISNULL(PINRequired ,0) , @LocationTimeRestrict= ISNULL( DayTimeRestrict,0) , @UseFor= ISNULL( UseFor,0)  From tblFacilityLocation with(nolock) where LocationID = @LocationID

---- Modify more on this
Select 	 @AgentID =F.AgentID,  @RateQuoteOpt = F.RateQuoteOpt, @FacDayRestrict  = F.DayTimeRestrict, @DebitOpt	= DebitOpt, @PINRequird = isnull(PINRequired,0), @DID = phone, @timeZone = isnull( F.timeZone,0),
		@isEnglish	=isnull(F.English ,1),@isSpanish= isnull( F.Spanish,0), @isFrench= isnull(F.French,0),@LiveOpt=isnull(  F.LiveOpt,0) ,  @PromptFileID= isnull( F.PromptFileID,2)  , @maxCallTime = maxCallTime,
...
Insert tblCallAttempt (FromNo ,    DialedNo, FacilityID   , AgentID)  values(@ANI,'00', @facilityID, @AgentID)
