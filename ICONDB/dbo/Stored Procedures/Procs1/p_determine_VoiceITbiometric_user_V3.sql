...
-- =============================================
CREATE PROCEDURE [dbo].[p_determine_VoiceITbiometric_user_V3]
@FacilityID int,
@InmateID		varchar(12),
@UserID		varchar(16),
...
BEGIN
	...
	SET @LanguageSelected = 0;
	 ...
	 if (select COUNT(*) from tblInmate with(nolock) where FacilityID =@FacilityID and InmateID=@InmateID and status = 1)  >0
		...
		select @LanguageSelected = isnull(LanguageSelected,'Uns') from tblBioMetricProfileVoiceITVerification with(nolock) where UserEmail = (@UserID + '@no.com') ;
