...
CREATE PROCEDURE [dbo].[p_get_calls_billed_by_cc_ICON3]
...
@TaxCallType tinyint  OUTPUT,
@totalFees numeric(4, 2) OUTPUT
AS
SET nocount on;
...
Begin
	...
	SET @totalFees =  @transFee  + @billSTFee;
	if(@callType ='ST')
		SET @TaxCallType =0;
	Else if(@callType in ('LT', 'AL','RL'))
		SET @TaxCallType =1
	Else
		SET @TaxCallType =2
