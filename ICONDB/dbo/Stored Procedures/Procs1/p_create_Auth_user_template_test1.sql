...
CREATE PROCEDURE [dbo].[p_create_Auth_user_template_test1]
@UserID varchar(30),
...
If ((select count(*) from tblUserRole where RoleDescript = @RoleDescript and facilityId = @facilityId) > 0 or
	(select count(*) from tblUserRole where RoleDescript = @RoleDescript and (facilityId = -1 or facilityId = -2)) > 0)
begin 
 Set @AuthId = -1
end
else
...
set @NewroleId = (SELECT MAX(roleId) FROM tblUserRole WHERE facilityId = @facilityId)
