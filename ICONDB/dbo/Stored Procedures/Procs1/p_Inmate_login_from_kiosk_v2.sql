...
-- =============================================
CREATE PROCEDURE [dbo].[p_Inmate_login_from_kiosk_v2] 
...
BEGIN
   ...
   Declare @VisitorName varchar(50),	@InmateName  varchar(50) ,	@AppDateTime	datetime ,	@AppDuration int ,	@SessionID	varchar(20)  ,@LocationName varchar(30), @FromTemplate int,
   ...
   If(@facilityID> 0)
    begin
		...
		if(@inmateID <> '0')
		 begin
			...
			if(@InmateLocationID is null or @InmateLocationID =0)
			 ...
			 begin
				if((@facilityID in ( 578,558,478)) and ((@locationID <> @InmateLocationID ) or @locationID=0) )
				 begin
					SET @inmateID ='0';
					SET @InmateName='';
					SET @InmateSO ='';
		...
		if(@FormOpt =1)
				select @MedicalKiteForm=MedicalKite, @InmateKiteForm = InmateKite,@GrievanceForm= Grievance ,@MedicalKiteReceiveEmail=MedicalKiteReceiveEmail, @InmateKiteReceiveEmail=InmateKiteReceiveEmail, @GrievanceReceiveEmail=GrievanceReceiveEmail, @MedProvider= isnull(MedProvider ,'Affordable care Act'), @FromTemplate = Template from tblfacilityForms where facilityID = @facilityID ;
    end
    ...
    @FormOpt as  FormOpt, @MedicalKiteForm as MedicalKiteForm, @InmateKiteForm  as InmateKiteForm , @GrievanceForm as  GrievanceForm, @MedicalKiteReceiveEmail as  MedicalKiteReceiveEmail , @InmateKiteReceiveEmail as InmateKiteReceiveEmail, @GrievanceReceiveEmail as  GrievanceReceiveEmail, @timeDiff as TimeDiff, @MedProvider as  MedProvider, @FacilityName as FacilityName,@FromTemplate as  FormTemplate;
