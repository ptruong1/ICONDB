...
-- =============================================
CREATE PROCEDURE [dbo].[p_get_Bio_Oxford_key_ProfileId_Inmate_V3]
...
BEGIN
	...
	set @BioInmateID = (Select top 1 InmateID from tblInmate where facilityID = @FacilityID and InmateID = @InmateID and Status = 1 order by InputDate)
	 ...
	 select @ProfileId=Isnull(profileId,'') from tblBioMetricProfileOxfordVerification with(nolock) where  userid = @userid ;
