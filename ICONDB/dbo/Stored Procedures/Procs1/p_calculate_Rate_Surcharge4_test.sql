...
CREATE PROCEDURE [dbo].[p_calculate_Rate_Surcharge4_test]
@ANI  varchar(10),
@DNI	varchar(20),
@FacilityID	int,
@billType char(2)	,
...
Declare @ANI_NPANXX char(6), @DNI_NPANXX char(6), @IsLocal smallint;
SET @subAgentID ='';
SET @firstMinute   =0;
SET @nextMinute   =0;
SET @connectFee = 0;
...
if(@facilityID in (1,0,610)  or @AgentID in ('1144','1283','1280','1157' ) )
 begin
  ...
  Begin
	...
	If(@billtype ='07' )
	 ...
	 Begin
		...
		If(@firstMinute =0 and @NextMinute =0 and  @connectFee =0 )  
		 Begin
			EXEC	p_get_rate_Detail2  	@ratePlanID	Output,	@Mileage	,@PointID	,@lataType ,'01'	,@dayCode,	@firstMinute	   OUTPUT,@NextMinute	   OUTPUT,@connectFee	    OUTPUT,@Minuteduration    OUTPUT, @Increment OUTPUT;
			EXEC  p_calculate_Surcharge_Fee  @SurchargeID, @Calltype,  @fromState	,  @PiP		Output , @libCode	OUTPUT;
			SET @duration =  @MinuteDuration;
		 End 
		If(@firstMinute =0 and @NextMinute =0 and  @connectFee =0 )  
...
Else   
 ...
 Begin
	...
	If(@billtype ='07'  )
	  Begin
		select  @firstMinute= firstMin , @NextMinute = NextMin, @connectFee= ConnectFee, @PiP= SurchargeFee, @increment= increment,  @duration= MinMinute  
			from tblDebitrate  with(nolock)  where RatePlanID = @rateplanID  and Calltype =4;
		If(@firstMinute =0 and @NextMinute =0 and  @connectFee =0 )  
		 Begin
    ...
    If(@billtype ='10'  )
	  Begin
		select  @firstMinute= firstMin , @NextMinute = NextMin, @connectFee= ConnectFee, @PiP= SurchargeFee, @increment= increment,  @duration= MinMinute  
			from tblPrepaidrate  with(nolock)  where RatePlanID = @rateplanID  and Calltype =4;
		If(@firstMinute =0 and @NextMinute =0 and  @connectFee =0 )  
		 Begin
			EXEC p_get_Intl_rate  @rateplanID, @DNI, @ConnectFee	  OUTPUT,@FirstMinute	  OUTPUT,@NextMinute  OUTPUT,@toState   OUTPUT,@toCity	  Output;
		 End 
		If(@firstMinute =0 and @NextMinute =0 and @connectFee =0 )  
		 ...
		 End
 	  End  
	else  If(@billtype in ('10','01' ,'03','05' ))
	 begin
		EXEC p_get_Intl_rate  @rateplanID, @DNI, @ConnectFee	  OUTPUT,@FirstMinute	  OUTPUT,@NextMinute  OUTPUT,@toState   OUTPUT,@toCity	  Output;
	  end
	else
