...
-- =============================================
CREATE PROCEDURE [dbo].[p_get_visitor_onsite] 
	@confirmID int,
	@AcctNo	   varchar(12),
...
BEGIN
   ...
   if(@AcctNo <>'0')
	   ...
	   @AppDateTime=  ApmDate + ApmTime , @facilityID = FacilityID,@RecordOpt = RecordOpt  from leg_Icon.dbo.tblVisitEnduserSchedule with(nolock)
   ...
   if(@VisitorID >0)
    Begin	
	   ...
	   SELECT @VisitorName= left(VFirstName,24) + ' ' + left(VLastName,24), @RecordOpt= isnull(RecordOpt,'Y')  FROM tblVisitors with(nolock) where VisitorID = @VisitorID ;
		   ...
		   if(@timePass < @AppDuration)
			begin
				...
				if(select COUNT(*) from tblVisitOnline where RoomID = @confirmID) = 0
					...
					Update tblVisitOnline SET  VisitorIP = @StationID,status =5,  VisitorLoginTime=GETDATE() where RoomID = @confirmID and InmateLoginTime is not null ;
				end
		   else
			begin
				SET @AppDuration=0;
