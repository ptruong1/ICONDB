...
-- =============================================
 CREATE PROCEDURE [dbo].[p_get_video_schedule_by_gender]
 @facilityID int,
	...
	SELECT @period = isnull(LimitTime,@period), @InterApm= isnull(InterApm,@InterApm)  , 
		   @FromTime1= isnull(fromTime1,''), @toTime1 = isnull(totime1,''),
		    @FromTime2= isnull(FromTime2,''), @toTime2=isnull(toTime2,''),
			 ...
			 from [tblvisitgenderschedule] with(nolock)
				   where FacilityID = @facilityID and scheduleday= @dw and gender=  @Gender  and visitType =@VisitType;
