CREATE PROCEDURE [dbo].[p_insert_billed_calls_webservice]
@callDate	varchar(6),
 ...
 begin
   ...
   */
	 ...
	 Begin
		if(@AgentID =7)
		 begin
			...
			select @CurrentBalance = PhoneBalance from tblAmtelLogs with(nolock) where ReferenceNo= @RecordID and APItype=1 ;
		else
			select @CurrentBalance  = balance  from tbldebit with(nolock) where AccountNo = @creditCardNo;	
		If(@AgentID <>102)
		begin
			...
			if(@CurrentBalance < @callRevenue )
			 begin
				SET @callRevenue = @CurrentBalance ;
			 end	
		end		
	 End	
	...
	Else if(@billtype ='08' or @billtype ='19')
	 begin
		 Update tblInmate set  FreeCallRemain=FreeCallRemain-1  where facilityID = @facilityID and PIN=@pin and InmateID=@inmateID and FreeCallRemain>0;
	 end

	 --- Move 
	if(@duration >2)
	  Begin
		if (@duration <30 AND (@AgentID not in (404,465,102)) and (@FacilityID not in (689,556,557,786)) and @billtype <>'08' ) 
			 SET @RejErrorcode=3 ;
		if (@duration <10  AND (@AgentID not in( 465,102)) and @billtype <>'08')
			  SET @RejErrorcode=3 ;
        if(@BillType in ('03','05') or len(@creditCardNo) >13)
		 begin
			Declare @CCEncr  varbinary(200);
		...
		If(@errorCode <10)
		 begin
			 ...
			 Values(@recordID,@Calldate, @ConnectTime, @FromNo ,@ToNo, @BillToNo ,'04', @billType,@callType, @FromState, @FromCity ,  
				@ToState, @ToCity,  @CallPeriod,  @creditCardType, @creditCardNo, @creditCardExp, @cvv,@zipcode, @settlementCode,
				@UserName,@RejErrorcode, @RateClass, @firstMinute,@addMinute,@connectFee,@Surcharge,@MinDuration,@recordDate,@ratePlanID,@ResponseCode,
				 @AgentID,@RecordFileName,@increment, '0',@folderDate	,@inmateID ,@PIN, @facilityID, @duration, @CallRevenue,@Pif,@CCEncr );
		 end
...
if(@@error =0)
  ...
  select '1'  as  SetBilledCalls, @RecordDest as RecordDest, @callRevenue as CallRevenue, @AmtelFacilityID as AmtelFacilityID, @recordDate  as recordDate ;

else
  select '0' as SetBilledCalls, @RecordDest as RecordDest,  @callRevenue as CallRevenue, @AmtelFacilityID  as AmtelFacilityID, @recordDate as recordDate ;

 If(@AgentID = 7)
 Begin
	SET @Duration = Ceiling((@Duration /60.00));
	if((@duration * 0.09) > @callRevenue )
	 Begin
		Set @duration = @duration -1;
		Set @callRevenue = @duration * 0.09;
	 end
