...
CREATE PROCEDURE [dbo].[p_create_Agent_user_12042018]
@UserID varchar(20),  
@Password  varchar(20),
@FacilityID int ,
@AgentID  int,
...
insert tblAuthUsers(AuthID, FacilityConfig, UserControl, PhoneConfig, CallControl ,DebitCard ,InmateProfile ,Report, CallMonitor ,Messaging, VideoVisit, ServiceRequest,Kites, MyReport,[Admin],PowerUser,[Finance-Auditor],Investigator,DataEntry,UserDefine)
		   values(@CurrentID, @FacilityConfig, @UserControl, @PhoneConfig, @CallControl ,@DebitCard ,@InmateProfile ,@Report, @CallMonitor ,@Messaging,@VideoVisit, @ServiceRequest,@Kites,@MyReport,@Admin,@PowerUser,@Finance_Auditor,@Investigator,@DataEntry,@UserDefine);
...
Insert tblUserprofiles (UserID ,  Password, FacilityID , AgentID, LastName ,  FirstName ,  MidName ,ID,  Department , authID, Phone , Email, userLevel, Createdby, IPaddress, UserIDDEC, PasswordDEC, SecondFactor,UserGroupID, MasterUserGroupID) 
values(@UserID ,@password, @FacilityID , @AgentID , @LastName  ,@FirstName   ,@MidName  , @ID, @Department  , @authID, @Phone  ,@Email,  @userLevel ,@Createby,@IPaddress,@EnCryptUser , @EnCryptPass, @SecondFactor,@UserGroupID, @MasterUserGroupID);
SET  @UserAction =  'Insert Agent User: ' + @UserID;
