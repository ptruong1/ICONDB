...
CREATE PROCEDURE [dbo].[p_get_prepaid_rate_quote_test]
...
Declare @pointID	varchar(2), @TotalSurcharge  numeric(4,2) , @libCode varchar(2), @CLEC_type  varchar(2)
Select @ANI = phone, @rateplanID = rateplanID, @MaxCallTime =   MaxCallTime from tblFacility with(nolock) where facilityID = @FacilityID	
...
If (@ConnectFee =3.99) 
 	select  @ConnectFee	=  CreditCardFee, @RatePerMin	=AddlMin  from  tblRateplanDetail  with(nolock)  where  RateID = @rateplanID and (type =@calltype  or Type='0')   and pointID = @pointID and  daycode=2


EXEC  p_calculate_Surcharge_Fee   @rateplanID,@CLEC_type,  @fromState	,  @TotalSurcharge   OUTPUT , @libCode	OUTPUT

select  @TotalSurcharge, @CLEC_type
