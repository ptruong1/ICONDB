...
-- =============================================
CREATE PROCEDURE [dbo].[p_get_appointment_info_rate_by_ApmNo1]
@ApmNo int
...
BEGIN
	...
	Select @VisistorName = VFirstName + ' ' + VLastName  from [leg_Icon].[dbo].tblVisitors where VisitorID =@VisitorID
	select  @totalCharge= ConnectFee + PerMinCharge *@LimitTime , @RateQuota = isnull(RateQuotaPerMonth,0) from [leg_Icon].[dbo].[tblVisitRate] where RateID=@facilityID  and visitType=@visitTypeID
	Select @VisitType = Descript  from [leg_Icon].[dbo].tblVisitType where VisitTypeID = @visitTypeID
	Select @facilityName = location from tblfacility where FacilityID = @facilityID 
	Select @LocationName = LocationName from tblVisitLocation where LocationID =@locationID
	if(@totalCharge =0)
		select  @totalCharge= ConnectFee + PerMinCharge *@LimitTime  from [leg_Icon].[dbo].[tblVisitRate] where RateID=1 and visitType=@visitTypeID
