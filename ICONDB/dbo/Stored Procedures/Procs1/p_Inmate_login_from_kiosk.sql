...
-- =============================================
CREATE PROCEDURE [dbo].[p_Inmate_login_from_kiosk] 
...
BEGIN
	...
	SET NOCOUNT ON;
	/*
   Declare  @MedicalKiteForm bit, @InmateKiteForm bit, @GrievanceForm bit,@facilityID int,  @inmateID varchar(12), @locationID int,@roomID int,@visitType tinyint,
   ...
   Declare @VideoVisitOpt tinyint, @EmailOpt tinyint, @VideoMessageOpt tinyint, @PictureOpt tinyint, @TextMessageOpt tinyint; 
   SET @facilityID =0 ;
   SET @inmateID ='0';
   SET @SoftphoneOpt =0;
   SET @FormOpt = 0 ;
   ...
   select @facilityID= a.FacilityID ,@locationID  =a.LocationID,@LocationName= b.LocationName  FROM [leg_Icon].[dbo].tblVisitPhone  a join [leg_Icon].[dbo].tblVisitLocation b on (a.FacilityID=b.FacilityID and a.LocationID =b.LocationID )
   ...
   If(@facilityID> 0)
    begin
		select @inmateID = InmateID,@InmateName = FirstName + ' ' + LastName , @DOB =isnull(DOB,'')  from tblInmate where FacilityId = @FacilityId and PIN =@PIN and [Status]=1;
		SELECT @InmateSO =bookingNo from tblInmateInfo where FacilityID =@facilityID and PIN=@inmateID 
		select @timeDiff = timezone from tblfacility where FacilityID =@facilityID ;
		if(@inmateID <> '0')
		 begin
			select @SoftphoneOpt = isnull(softphoneOpt,0), @FormOpt = isnull(FormsOpt,0) from tblFacilityOption where FacilityID =@FacilityId ;
			update tblVisitInmateConfig set LocationID = @locationID where FacilityID =@facilityID and InmateID =@inmateID 
		 end
		if(@FormOpt =1)
				select @MedicalKiteForm=MedicalKite, @InmateKiteForm = InmateKite,@GrievanceForm= Grievance ,@MedicalKiteReceiveEmail=MedicalKiteReceiveEmail, @InmateKiteReceiveEmail=InmateKiteReceiveEmail, @GrievanceReceiveEmail=GrievanceReceiveEmail  from tblfacilityForms where facilityID = @facilityID ;
   ...
   If(@facilityID> 0)
		...
		if(@inmateID <> '0')
		 begin
			...
			if(@InmateLocationID is null or @InmateLocationID =0)
			 ...
			 begin
				if((@facilityID in( 578,478,558) ) and (@locationID <> @InmateLocationID) )
				 begin
					SET @inmateID ='0';
					SET @InmateName='';
					SET @InmateSO ='';
    ...
    @FormOpt as  FormOpt, @MedicalKiteForm as MedicalKiteForm, @InmateKiteForm  as InmateKiteForm , @GrievanceForm as  GrievanceForm, @MedicalKiteReceiveEmail as  MedicalKiteReceiveEmail , @InmateKiteReceiveEmail as InmateKiteReceiveEmail, @GrievanceReceiveEmail as  GrievanceReceiveEmail, @timeDiff as TimeDiff;
