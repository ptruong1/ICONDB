...
CREATE PROCEDURE [dbo].[INSERT_DNIWatchListDetailsWithAlert2]
(	
	@FacilityID int,
	@watchListID int,
	@DivisionID int,
...
AS
	...
	BEGIN
        ...
        set           @ID = @nextID ; 
		INSERT INTO [tblWatchList] ([ID],[watchListID], LocationID, DivisionID, [ANI], [DNI], [PIN], [modifyDate], [Username], [WatchByID],[ThirdPartyAlert],[LocationTrace]) 
		VALUES (@ID, @watchListID, @LocationID, @DivisionID, @ANI, @DNI, @PIN, getdate(), @Username, @WatchByID,@ThirdPartyAlert,@LocationTrace);
		INSERT INTO [tblAlertPhones] ([PhoneNo],[FacilityID],[Username],[AlertEmails],[AlertCellPhones],[AlertRegPhone],[HourlyFreq],[DailyFreq],[WeeklyFreq],[MonthlyFreq],[WatchListDetailID],[AlertMessage])
			   VALUES (@DNI,@FacilityID,@Username,@AlertEmails,@AlertCellPhones,@AlertRegPhone,@HourlyFreq,@DailyFreq,@WeeklyFreq,@MonthlyFreq,@ID,@AlertMessage);
		RETURN 0;
	END
