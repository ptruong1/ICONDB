CREATE PROCEDURE [dbo].[p_insert_FacilityVisitSchedule]
(
	@FacilityID int,
	@LocationID int,
	@UserName varchar(25),
...
WHERE FacilityID = @FacilityID AND ScheduleDay = @ScheduleDay)
	BEGIN
		...
		SET [FromTime1] = @FromTime1, 
			...
			[userName] = @userName, 
			[modifydate] = GETDATE()
		 WHERE (([FacilityID] = @FacilityID) AND (ScheduleDay = @ScheduleDay) and (FvisitType=1));
		...
		SET [FromTime1] = @RFromTime1, 
			...
			[userName] = @userName, 
			[modifydate] = GETDATE()
		 WHERE (([FacilityID] = @FacilityID) AND (ScheduleDay = @ScheduleDay)and (FvisitType=2));
