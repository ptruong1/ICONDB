...
CREATE PROCEDURE [dbo].[p_insert_CallRecord5]
@callDate	varchar(6),
...
@RecordID		bigint
 AS
SET NOCOUNT ON
Declare  @YY  char(2), @MM char(2), @tblCall  varchar(20),  @SQL  varchar(500),  @callPeriod  char(1), @HH smallint, @trunKID varchar(4) ,@settlementCode	char(1), @phoneType int
...
If ( @responseCode ='050' )
	SET @phoneType =1
else if  ( @responseCode ='399'  or   @responseCode ='283' )
	SET @phoneType =2
else 	
	SET @phoneType =3
...
set @recordDate = dateadd(hh, @timeZone, @recordDate) 
 ...
 INSERT INTO  tblOnCalls( RecordID, Calldate, ConnectTime, FromNo ,   ToNo, BillToNo ,  MethodOfRecord,billType,callType, FromState, FromCity ,  
	...
	AgentID, RecordFile, MinIncrement, channel, folderDate, InmateID,PIN,facilityID, InRecordFile,phoneType	)
 Values( @RecordID, @Calldate, @ConnectTime, @FromNo ,@ToNo, @BillToNo ,  @MethodOfRecord,@billType,@callType, @FromState, @FromCity , 
	...
	@ProjectCode ,@UserName,@errorCode, @RateClass, @firstMinute,@addMinute,@connectFee,@Pip,@MinDuration,@recordDate,@ratePlanID,@DbError, @ResponseCode,
	 @AgentID,@RecordFileName,@increment, @channel,@folderDate	,@inmateID,@PIN, @facilityID, @InRecordFile, @phoneType)
...
Else
  Begin
	If(@errorCode =  9)  SET  @errorCode  =2
	...
	If(@errorCode = 6)
	  ...
	  End
  End
--If( @billtype = '01'  And  @responseCode <> '' )
	--exec p_update_LiB_reply   @billToNo, @responseCode
