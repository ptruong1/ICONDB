...
-- =============================================
CREATE PROCEDURE [dbo].[p_determine_device_facilityOpt]
@EdoFacilityID	int,
@AssetID		varchar(25)
...
BEGIN
   SET NOCount ON;
	...
	select @facilityID= facilityID  from tblTablets with(nolock) where tabletID=  @AssetID and status=1
	If(@facilityID >0)
	 Begin    
		...
		@CommissaryOpt = isnull(commissaryOpt,0),@MediaOpt =isnull( MediaOpt,0),  @HandbookOpt= isnull(HandbookOpt,0) from tblFacilityOption with(nolock) where FacilityID = @facilityID;
			...
			If(  @EmailOpt =1) 
				SET @EmailSite = 'http://legacyinmate.com/LegacyMessagingNoKeypad/InmateForms/EmailMessages.aspx';
			If(@VideoMessageOpt =1) 
				SET @VideoMessageSite = 'http://legacyinmate.com/LegacyMessagingNoKeypad/InmateForms/VideoMessages.aspx';
			If(@formsOpt =1) 
				SET @FormSite = 'http://legacyinmate.com/visitationLassen/InmateForms/InmateFormsMenu.aspx';
