...
CREATE PROCEDURE [dbo].[p_determine_Facility_Web_service_v2] --- New Edit for language
@ANI	varchar(10) 
 AS
SET NOCOUNT ON;
...
Declare @facilityID  int  ,@PINRequird tinyint  ,@AgentID  int ,@isEnglish	tinyint ,@isSpanish	tinyint  ,@isOther tinyint  ,@LiveOpt	tinyint ,@PromptFileID   smallint ,@RateQuoteOpt	tinyint , @PreaLine varchar(32), @TipLine varchar(32),
	@maxCallTime	smallint  ,@DebitOpt	tinyint ,@IncidentReportOpt tinyint ,@BlockCallerOpt tinyint ,@CollectwithCC	tinyint  ,@isprobono	tinyint ,@OverlayOpt	tinyint ,  @PreaLine2 varchar(10), @TipLine2 varchar(10),  @PreaLine3 varchar(10), @TipLine3 varchar(10),
	@TTYOpt	tinyint ,@DID		varchar(10) ,@UseFor	tinyint  ,@commOpt	tinyint ,@HotLine varchar(32),@HotLine1 varchar(10),@HotLine2 varchar(10), @CommissaryPhone varchar(40),@probonoPhone varchar(10),@AccessTypeID  tinyint, @languages varchar(300), @DirectPhoneNo varchar(10);
SET @facilityID = 0;
...
If (@UseFor =6)
 begin
	Select @DirectPhoneNo =ISNULL( DirectNo,'0') from tblDirectphones where facilityID = @facilityID and (ANI = @ANI or ANI is null);
	if(@DirectPhoneNo ='0')
		SET @UseFor  =0;
	Else
		goto SelectOUT;
...
SELECT @HotLine = isnull(phoneNo,'0'),@HotLine1 = isnull(phoneNo1,'0'),@HotLine2 = isnull(phoneNo2,'0'),
		@PreaLine =isnull(Prea1,'0') , @PreaLine2= isnull(prea2,'0'), @PreaLine3 =isnull( prea3,'0'),
		@TipLine =isnull(Tip1,'0') , @TipLine2= isnull(tip2,'0'), @TipLine3 =isnull( Tip3,'0') from tblHotLines with(nolock)  where FacilityID=@facilityID;

if(@HotLine1 <> '0') SET @HotLine = @HotLine + '-' + @HotLine1;
if(@HotLine2 <> '0') SET @HotLine = @HotLine + '-' + @HotLine2;

if(@PreaLine2 <> '0') SET @PreaLine =  @PreaLine + '-' +  @PreaLine2;
if(@PreaLine3 <> '0') SET @PreaLine =  @PreaLine + '-' +  @PreaLine3;

if(@TipLine2 <> '0') SET @TipLine =  @TipLine + '-' +  @TipLine2;
if(@TipLine3 <> '0') SET @TipLine =  @TipLine + '-' +  @TipLine3;



If(@isOther=1)
 begin
	select @languages = @languages + '_' + isnull(Languages,'') from tblFacilityOption with(nolock) where FacilityID =@facilityID ;
 end
...
select @facilityID as  facilityID ,@PINRequird as  PINRequird ,@AgentID as AgentID ,@isEnglish as 	English ,@isSpanish as Spanish ,@isOther as French  ,@LiveOpt	as LiveOpt, @PromptFileID  as PromptID,
	...
	@BlockCallerOpt as BlockCallerOpt  ,@CollectwithCC as  CollectwithCC ,@probonoPhone as  Probono, @OverlayOpt as OverlayOpt , @TTYOpt as  TTYOpt, 
	 @CommissaryPhone as  CommOpt, @DID as PhoneID, @UseFor as	InfoOpt , @HotLine as Hotline,@AccessTypeID  as AccessTypeID, @languages as Languages, @IDReq as IDRequired, @phoneStatus as PhoneStatus, @PreaLine as PreaLine, @TipLine as TipLine, @DirectPhoneNo as DirectPhoneNo ;
