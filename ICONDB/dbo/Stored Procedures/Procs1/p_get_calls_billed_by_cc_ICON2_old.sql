...
CREATE PROCEDURE [dbo].[p_get_calls_billed_by_cc_ICON2_old]
 ...
 Begin	
	select  top 1  @FromNo=     FromNo,  @AgentID = AgentID,  @facilityID = facilityID,
		...
		where  complete in ('1','0')  and  (billType  =  '03'  or   billType  =  '05' )    and duration > 0  and CallRevenue >1
		 ...
		 and creditCardNo not in (select ccNo from [Tecodata].[dbo].[tblOfficeCard]  With (nolock))
		  and left(creditcardno,16) not in 	(select Cnum from TecoData.dbo.tblBCresponse R  With (nolock) where   C.calldate = R.Calldate and C.connecttime =R.Calltime and datediff(d,transDate,getdate()) < 3  ) ;		
		if( @RecordID >0)
