...
-- =============================================
CREATE PROCEDURE [dbo].[p_determine_kiosk_facilityOpt_080317]
@KioskName	Varchar(15),
@IPaddress  Varchar(16)
AS
BEGIN
   SET NOCount ON;
	Declare @facilityID int, @facilityName as varchar(200),@VideoVisitOpt bit, @VideoVisitSite varchar(300), @FormsOpt bit, @FormSite varchar(300),@DeviceType tinyint,
	...
	if(@facilityID >0)
	 begin
		SET @DeviceType =1; --- For Video Kiosk
	 end
	else 
	 begin
    ...
    if(@facilityID >0)
	 begin
		...
		if( @MediaOpt =1)
		 ...
		 If( @VideoVisitOpt =1)
		 	SET @VideoVisitSite = 'https://www.legacyvisit.com:5080/Chat/index.html?RoomID=88&UserID=myID1&AnotherID=myID2&UserType=C&SessionID=88&Duration=30&Record=Y&TextChat=N&videoDisplayW=960&videoDisplayH=720'; 
			--SET @VideoVisitSite = 'https://legacyinmate.com/visitationLassen/inmatelogin.aspx';
		  If(  @EmailOpt =1) 
			SET @EmailSite = 'https://legacyinmate.com/visitationLassen/InmateForms/emailmessages.aspx';
		  If(@VideoMessageOpt =1) 
			SET @VideoMessageSite = 'https://legacyinmate.com/visitationLassen/InmateForms/Videomessages.aspx';
		  If(@formsOpt =1) 
         ...
         If (@PictureOpt =1)
			SET @PictureSite = 'http://bcd.legacyinmate.com/LegacyMessaging/InmateForms/PictureSharing.aspx';
