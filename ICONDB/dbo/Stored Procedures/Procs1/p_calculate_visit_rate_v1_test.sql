...
-- =============================================
CREATE PROCEDURE [dbo].[p_calculate_visit_rate_v1_test]   --- Need Schedule Date
@FacilityID int , 
@InmateID varchar(12) , 
@visitTypeID tinyint, 
@VisitorID int,
...
AS
BEGIN
	Declare  @RateQuota smallint, @RelationshipID tinyint, @promo numeric(4,2), @FreeVisitPerWeek smallint;
	SET @totalCharge =0;
	SET @RateQuota =0;
	SET @RelationshipID =1;
	SET  @promo =1;
	SET @FreeVisitPerWeek  =0;
	Select @RelationshipID =RelationshipID  from [leg_Icon].[dbo].tblVisitors with(nolock) where VisitorID =@VisitorID;

	select  @totalCharge= ConnectFee + PerMinCharge *@LimitTime , @RateQuota = isnull(RateQuotaPerMonth,0), @FreeVisitPerWeek = isnull(FreePerWeek,0) from [leg_Icon].[dbo].[tblVisitRate] with(nolock) where RateID=@facilityID  and visitType=@visitTypeID;
	...
	If([dbo].[fn_getFreeVisitPerWeekByInmateID_facility] (@InmateId,  @scheduleDate,  @facilityID ,@visitTypeID  ) < @FreeVisitPerWeek)
		SET  @totalCharge =0;
