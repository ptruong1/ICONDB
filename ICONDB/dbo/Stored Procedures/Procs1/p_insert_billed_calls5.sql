CREATE PROCEDURE [dbo].[p_insert_billed_calls5]
@callDate	varchar(6),
...
If(@billtype =  '06')
 ...
 begin
	...
	If(@billtype = '07'  and @duration >5 ) 
	 Begin
		
		SET @callRevenue = dbo.fn_CalculateCallRevenue(@firstMinute  ,@addMinute ,@connectFee ,@duration ,@callType  ,@PIP  ,@minDuration ) 
		Update tbldebit SET balance = balance - @CallRevenue, modifydate = getdate(),status=1 where AccountNo = @creditCardNo	
