...
-- =============================================
CREATE PROCEDURE [dbo].[p_determine_Oxfordbiometric_Inmate_V2]
@FacilityID int,
@InmateID		varchar(12),
@UserID		varchar(16),
...
BEGIN
	...
	SET @RemainEnrollments = 9;
	 ...
	 if (select COUNT(*) from tblInmate with(nolock) where FacilityID =@FacilityID and InmateID=@InmateID and status = 1)  >0
		...
		set @LanguageSelected = (select  Abbrev from tblLanguages with(nolock) where AcpSelectOpt = (Select primaryLanguage from tblInmate 
