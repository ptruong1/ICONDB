...
CREATE PROCEDURE [dbo].[p_Chart_Agent_Dashboard_EmailVoiceVideoMessage_ByHour]
@AgentID  int
 AS
 ...
 where facilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1) and MessageTypeID =1 and 
...
t1.FacilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1) and t2.MessageTypeID = '2'

Union all
 select  DATEPART(hour,t2.MessageDate) as Hours
from tblMailboxDetailF t2  with(nolock) 	left join tblMailBox  t1  with(nolock) on t1.MailBoxID = t2.SenderMailBoxID	

Where   convert(varchar(10), t2.MessageDate, 102)  = convert(varchar(10), getdate(), 102)  and t1.FacilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1) and t2.MessageTypeID = '2' ) as tem
group by Hours
 ...
 from tblMailboxDetail t2  with(nolock) 	left join tblMailBox  t1  with(nolock) on t1.MailBoxID = t2.MailBoxID 	
 where facilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1) and MessageTypeID =4 and
   convert(varchar(10), t2.MessageDate, 102)  = convert(varchar(10), getdate(), 102) 
 group by  DATEPART(hour,t2.MessageDate)
