CREATE PROCEDURE [dbo].[p_determine_ANI_DNI]
@ANI	char(10),
@DNI	varchar(16),
@PIN		varchar(12),
@facilityID	int,
...
Declare   @timeZone tinyint , @localtime datetime, @bookingTime datetime, @PANNotAllow bit,  @DNIRestrict  bit, @AgentID int
SET  @RecordOPT	='Y'
SET   @ReasonID =0
SET  @CallLimit  =0
SET @Freetype  =0 
...
Select  @PANNotAllow = PANNotAllow ,  @DNIRestrict  =DNIRestrict  from tblInmate with(nolock)  where facilityID = @facilityID and PIN =  @PIN
If (@DNIRestrict =  1)
 Begin
...
If  (( select count(*) from tblNonRecordPhones with(nolock) where phoneno = @DNI  and facilityID = @facilityID) > 0 or @facilityID = 76 or @facilityID=278   or  @AgentID =404 ) 
		SET @RecordOPT ='N'
If (@Billtype <>'07')
 Begin
	...
	If( select count(*) from tblFreePhones with(nolock) where phoneNo =  @DNI and facilityID= @facilityID ) > 0
	  Begin
		...
		Return 0
	  End
	select @Freetype =Freetype, @calls = isnull(calls,0) from tblFreeANIs  with(nolock)  where ANI =  @ANI	
	If ( @Freetype = 1)
	 Begin
