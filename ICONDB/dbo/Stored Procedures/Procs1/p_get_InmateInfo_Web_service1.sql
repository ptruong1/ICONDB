CREATE PROCEDURE [dbo].[p_get_InmateInfo_Web_service1]
@PIN  varchar(12),
@fromNo	char(10),
@facilityID	int, 
@userName	varchar(23),
...
Declare @DNIRestrict    tinyint, @AlertEmail        varchar(25),@AlertPage  	Varchar(25)  ,@AlertPhone     Varchar(25),@InmateFirstName	 Varchar(25),@InmateLastName	varchar(25) ,  @SusDate date,@visitphone tinyint,
	@InmateStatus		tinyint ,@Register tinyint ,@AcessAllow  smallint,  @inmateID varchar(12), @AccuPIN tinyint, @BioMetric tinyint,@SuspendStart smalldatetime,@SuspendEnd smalldatetime , @DebitOpt tinyint,
...
SET @DebitOpt  =0;
SET @visitphone =0;
select @timeZone =isnull( timezone,0), @DebitOpt = isnull(DebitOpt,0) from tblfacility with(nolock) where facilityID =@facilityID ;
...
select @InmateFromNo= Fromno from tblinmateoncall with(nolock) where PIN=@PIN and facilityID = @facilityID  and  Accesstime > dateadd(MINUTE,-3,GETDATE());
--select @InmateFromNo;
...
If( @InmateStatus=1)
  Begin
	...
	if(@PIN <> '0' and  @PIN<> '9999420' AND @TTY =0)  
	 begin
	...
	If (@PIN in ('9999420','765602442','1475635527','123123','1219828535','1117395961'))
		set @BioMetric =0;
  End
Else 
 Begin
...
if(@DebitOpt =1) --- New Update if debit opt than select account
 begin
	...
	If(@DebitBalance <= 0.30 and @facilityID in (43,786,556,557))
		SET @DebitAcct ='0';
...
if(@AccuPIN =1)
 begin
	...
	if(@dayBirth<>'' and  @MonthBirth <> '' and  @yearBirth  <>'' and @first4BookID <>'' and  @last3BookID <>'')
