...
-- =============================================
create PROCEDURE [dbo].[p_get_visit_videos_09192018] 
@facilityID int,
@fromDate varchar(10),
@toDate	  varchar(25),
@InmateID varchar(12),
...
AS
 ...
 Begin
  ...
  (Select top 1 PIN from tblInmate B where A.FacilityID = B.FacilityId and A.InmateID = B.InmateID order by ModifyDate desc) as PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	  ...
	  , A.StationID, L.LocationName, L.LocationID, D.VisitorID, isnull(D.VImage,' ') as VImage, A.CreatedBy, A.ApprovedBy, A.CancelBy
  FROM [leg_Icon].[dbo].[tblVisitEnduserSchedule] A 
  left join tblVisitLocation L on L.locationID = A.locationID  -- (select LocationID from [tblVisitInmateConfig] ic where ic.facilityId = @facilityId and ic.InmateID = @inmateId) 
 ...
 If @InmateID <> '' -- Level 1
  ...
  (Select top 1 PIN from tblInmate B where A.FacilityID = B.FacilityId and A.InmateID = B.InmateID order by ModifyDate desc) as PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	   A.VisitType, X.Descript, InmateLogInID, VisitorLogInID, Note, isNull(AlertCellPhone,'') as AlertCellPhone,  
	  isNull(AlertCellCarrier,'') as AlertCellCarrier, Y.Descript as ApptStatus, D.Phone1, ISNULL(TotalCharge,0) as TotalCharge, A.RecordOpt, ISNULL(A.RecordStatus,0) as RecordStatus, C.ChatServerIP as ChatServerIP
	  , A.StationID, L.LocationName, L.LocationID, isnull(D.VImage,' ') as VImage, A.CreatedBy, A.ApprovedBy, A.CancelBy
  FROM [leg_Icon].[dbo].[tblVisitEnduserSchedule] A 
  left join tblVisitLocation L on L.locationID = A.locationID --  (select LocationID from [tblVisitInmateConfig] ic where ic.facilityId = @facilityId and ic.InmateID = @inmateId) 
...
, A.StationID, L.LocationName, L.LocationID, D.VisitorID, isnull(D.VImage,' ') as VImage, A.CreatedBy,  A.ApprovedBy, A.CancelBy
  ...
  (Select top 1 PIN from tblInmate B where A.FacilityID = B.FacilityId and A.InmateID = B.InmateID order by ModifyDate desc) as PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	   A.VisitType, X.Descript, InmateLogInID, VisitorLogInID, Note, isNull(AlertCellPhone,'') as AlertCellPhone,  
	  isNull(AlertCellCarrier,'') as AlertCellCarrier, Y.Descript as ApptStatus, D.Phone1, ISNULL(TotalCharge,0) as TotalCharge, A.RecordOpt, ISNULL(A.RecordStatus,0) as RecordStatus, C.ChatServerIP as ChatServerIP
	  , isnull(D.VImage,' ') as VImage, A.CreatedBy,  A.ApprovedBy, A.CancelBy
...
, A.StationID, L.LocationName, isnull(D.VImage,' ') as VImage, A.CreatedBy,  A.ApprovedBy, A.CancelBy
  FROM [leg_Icon].[dbo].[tblVisitEnduserSchedule] A 
  left join tblVisitLocation L on L.locationID =  A.locationID --- (select LocationID from [tblVisitInmateConfig] ic where ic.facilityId = @facilityId and ic.InmateID = @inmateId) 
  Left Join [leg_Icon].[dbo].[tblVisitType] X 
	on A.visitType = X.VisitTypeID
