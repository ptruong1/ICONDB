CREATE PROCEDURE [dbo].[p_get_billtype_web_service_V2_test]
@ANI	char(10),
@DNI	varchar(16),
...
If(@billtype ='07' )  
 begin
	...
	-- select @AcessAllow, @balance;
	if(@AcessAllow <=0 or @balance <@Minbilled)  -- Convert to prepaid
	  begin
			Select @balance = balance,  @isPrepaid =status   From tblPrepaid with(nolock) where  phoneNo = @DNI and status =1;
			if (@balance >0)
					SET @billtype ='10';	
