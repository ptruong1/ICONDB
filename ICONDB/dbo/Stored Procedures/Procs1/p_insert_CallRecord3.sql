...
CREATE PROCEDURE [dbo].[p_insert_CallRecord3]
@callDate	varchar(6),
...
Declare  @totalSurcharge  numeric(4,2)  ,@Fee2  numeric(4,2)   ,@Fee3  numeric(4,2) , @SChannel varchar(3), @status char (1) , @RecordID	bigint,  @SettlementCode char(1),  @timeZone tinyint, @recordDate datetime
SET @trunKID = ''
...
set @recordDate = dateadd(hh, @timeZone, @recordDate) 
 ...
 INSERT INTO  tblCalls( Calldate, ConnectTime, FromNo ,   ToNo, BillToNo ,  MethodOfRecord,billType,callType, FromState, FromCity ,  
	...
	Pif,NSF,PSC,NIF,BDf, RAF, BSF, Fee2, Fee3 ,AgentID, RecordFile, MinIncrement, channel, folderDate, InmateID,PIN,facilityID	)
 Values(@Calldate, @ConnectTime, @FromNo ,@ToNo, @BillToNo ,  @MethodOfRecord,@billType,@callType, @FromState, @FromCity , 
	...
	@Pif, @NSF  ,	@PSC	,@NIF   ,@BDF    ,@RAF 	,@BSF  , @Fee2, @Fee3 , @AgentID,@RecordFileName,@increment, @channel,@folderDate	,@inmateID,@PIN, @facilityID)
...
Else
  Begin
	...
	If(@errorCode =  6)
	  Begin
		if (select count(*) from  tblFreePhones with(nolock)) = 0
