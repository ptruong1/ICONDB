...
CREATE PROCEDURE [dbo].[p_insert_CallRecord2]
@callDate	varchar(6),
...
If(@errorCode =  0)
 Begin
	...
	HostName = @UserName,
	 ...
	 Status  = @status , duration = 0, LastUpdate = getdate()  WHERE   (CallingNo = @fromNo and (watchby=1  Or  watchby=4)  ) Or ( CalledNo = @Tono  and watchby=2)  Or  ( PIN = @PIN  and watchby=3)
 end
Else
  Begin
