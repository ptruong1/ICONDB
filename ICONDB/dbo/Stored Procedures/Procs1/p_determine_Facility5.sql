...
CREATE PROCEDURE [dbo].[p_determine_Facility5]
@ANI	varchar(10)  ,
...
SELECT  @facilityID =  facilityID, @DivisionID=ISNULL( DivisionID,0), @locationID=ISNULL( LocationID,0),@ANIdayRestrict=ISNULL( DayTimeRestrict,0), @AniIDReq=ISNULL(IDrequired,0),  @AniPINReq  =ISNULL(PINRequired ,0),
	   @UseFor =isnull( InforOpt,0 )  , @isFree = isnull(isFree,0), @DebitOpt = isnull(DebitOpt,0)
...
if(@DebitOpt =0 ) 
 ...
 begin
	Select 	 @AgentID =F.AgentID,  @RateQuoteOpt = F.RateQuoteOpt, @FacDayRestrict  = F.DayTimeRestrict,  @PINRequird = isnull(PINRequired,0), @Fphone = phone, @timeZone = isnull( F.timeZone,0),
