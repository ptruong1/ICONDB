...
CREATE PROCEDURE [dbo].[p_Chart_Agent_Dashboard_CallVideo_ByHour]
@AgentID  int 
AS
...
where  AgentID=@AgentID  and  errorcode = '0' and
       convert(varchar(10), RecordDate, 102)  = convert(varchar(10), getdate(), 102) and
...
where facilityID in (select facilityID from  tblFacility  with(nolock) WHERE AgentID = @AgentID and status =1) and 
