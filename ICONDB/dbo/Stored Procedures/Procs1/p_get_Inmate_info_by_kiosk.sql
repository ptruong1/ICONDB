...
-- =============================================
CREATE PROCEDURE [dbo].[p_get_Inmate_info_by_kiosk] 
	@PublicIP varchar(16),
...
BEGIN
   ...
   Declare @VisitorName varchar(50),	@InmateName  varchar(50) ,	@AppDateTime	datetime ,	@AppDuration int ,	@SessionID	varchar(20) ,
	@VisitorID	int ,	@RecordOpt  char(1) ,	@TextOpt    char(1),	@RemainTime bigint , 	@ChatServerIP varchar(100) ;
   ...
   if (@inmateID ='')
    ...
    begin
		if (select COUNT(*) from tblVisitInmateConfig with(nolock)  where FacilityID = @FacilityID and InmateID = @inmateID) >0
			Update tblVisitInmateConfig set LocationID = @LocationID,ModifyDate =GETDATE() where FacilityID = @FacilityID and InmateID = @inmateID  and (locationID =0 or locationID is null);
		else
			if(@FacilityID >0)	
	...
	if(@AppDateTime='1/1/2000')
	 ...
	 begin
		...
		Update leg_Icon.dbo.tblVisitEnduserSchedule SET  InmateLogInID = @PublicIP where  ApmNo=@SessionID;
		...
		if(select COUNT(*) from tblVisitOnline where RoomID = @roomID) = 0
			...
			Update tblVisitOnline SET  StationID = @StationID,locationID = @locationID, status =5, InmateLoginTime=GETDATE() where RoomID = @roomID and VisitorLoginTime is not null;
	 end	
	EXEC  p_insert_kiosk_activity 	@PublicIP ,	1,	@FacilityID,	@InmateID ;
	Select @VisitorName as VisitorName,	@InmateName as  InmateName,	@AppDateTime	as AppDateTime ,@AppDuration as AppDuration ,	
			@SessionID	as RoomID ,	@VisitorID as VisitorID ,	@RecordOpt  as RecordOpt ,	@TextOpt as TextOpt,@RemainTime as RemainTime , @ChatServerIP as ChatServerIP ;
