...
CREATE PROCEDURE [dbo].[p_get_recording_transcriptRequest_v1_Orig]
@lastRecorID bigint, 
...
SET NOCOUNT ON;
SET  @RecordID =0;
if ( @CallerText is null)
  SET @CallerText ='';
if ( @CalleeText is null)
  SET @CalleeText ='';
if(@keyWords is null)
  SET @keyWords ='';
...
if(@lastRecorID >0)
 Begin
	if(select count(*) from leg_Icon.dbo.tblRecordingTransMatch where RecordID =@lastRecorID) =0
		Insert leg_Icon.dbo.tblRecordingTransMatch ( RecordID, WordsMatch,CalleeText,CallerText) values(@lastRecorID,@WordsMatch,@CalleeText,@CallerText);  -- match files
	else
		update leg_Icon.dbo.tblRecordingTransMatch set WordsMatch =WordsMatch + @WordsMatch, CalleeText= CalleeText + @CalleeText,CallerText= CallerText + @CallerText  where RecordID = @lastRecorID;
