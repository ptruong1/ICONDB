...
CREATE PROCEDURE [dbo].[p_calculate_Rate_Surcharge4]
@ANI  varchar(10),
@DNI	varchar(20),
@FacilityID	int,
@billType char(2)	,
...
if(@FacilityID=1) Select  @ratePlanID  =   isnull(RATEPLANID,@ratePlanID), @SurchargeID =  isnull(SurchargeID,@SurchargeID) from tblANIOp with(nolock) WHERE ANI= @ANI;
...
if( len(@DNI)=10 and  @DNI_NPA <>'011')
  Begin
    ...
    --- Modify for free calll 
	If(@billtype ='08' or  @billtype ='13' or @billtype ='17' )
	 begin
			return 0;
	 end
	if( @DNI_NPA in ('800','888','866','877','900') or  @DNI_NXX = '555'  )
	begin
