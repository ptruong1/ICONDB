CREATE PROCEDURE [dbo].[p_get_InmateInfo_by_InmateID_Web_service]
@InmateID varchar(12),
@fromNo	char(10),
@facilityID	int, 
@userName	varchar(23),
...
Declare  @i int , @DateTimeRestrict  bit,  @PIN varchar(12),   @localtime  smalldatetime,   @day int,   @h  int  ,@timeZone smallint, @CallPerHour smallint, @CallPerDay smallint , @CallPerWeek smallint, @CallPerMonth smallint ,@TTY tinyint;
Declare @DNIRestrict    tinyint, @AlertEmail        varchar(25),@AlertPage  	Varchar(25)  ,@AlertPhone     Varchar(25),@InmateFirstName	 Varchar(25),@InmateLastName	varchar(25) ,  @SusDate date,  @PINChangeOpt tinyint,
...
select  @PIN = PIN,  @AlertEmail=isnull(AlertEmail,''),  @AlertPage =isnull( AlertPage,''), @AlertPhone= isnull(AlertPhone,'') , @DateTimeRestrict = DateTimeRestrict , @DNIRestrict=DNIRestrict   , @InmateStatus	 = status,@InmateFirstName= FirstName,
			@InmateLastName = LastName,	@CallPerHour  = Isnull(MaxCallPerHour,0) , @CallPerDay= isnull( MaxCallPerDay,0),  @CallPerWeek =isnull(  MaxCallPerWeek,0),  @CallPerMonth =isnull( MaxCallPerMonth ,0) ,  @TTY = isnull(TTY,0),
			 @SuspendStart = StartDate, @SuspendEnd = EndDate, @AssignToDivision = AssignToDivision,  @AssignToLocation= AssignToLocation,  @AssignToStation= AssignToStation,@NameRecorded = isnull(NameRecorded,0), @FreeCallNo= isnull(FreeCallRemain,0)
			From tblInmate  with(nolock)  where   InmateID = @InmateID and   facilityID =@facilityID and [Status]=1;

select @InmateFromNo= Fromno from tblinmateoncall with(nolock) where InmateID=@InmateID and facilityID = @facilityID  and  Accesstime > dateadd(MINUTE,-@MaxTimeCall,GETDATE());
...
if((@InmateFromNo is not null and @InmateFromNo <> @fromNo) and @visitphone=0)
 begin
...
If( @InmateStatus=1)
  Begin		
	if(@PIN not in( '0','9999420')  AND @TTY =0)  
		 insert tblinmateOncall (PIN,FacilityID,AccessTime,FromNo, InmateID) values(@PIN,@facilityID ,GETDATE(), @fromNo, @inmateID ); 
	Select @AccuPin =isnull(AccuPin ,0),@BioMetric= isnull( BioMetric,0), @VoiceMessage= isnull(VoiceMessageOpt,0) ,  @PINChangeOpt = isnull(PINChangeOpt,0) From leg_Icon.dbo.tblFacilityOption with(nolock) where FacilityID = @facilityID;
    
	--USING Hardcode FOR LEGACY TEST 
...
Select  @PIN as PIN, @AcessAllow as AccessAllow , @InmateFirstName as FirstName,@InmateLastName as LastName , @InmateStatus  as Status ,@Register as Register  , @DNIRestrict   as DNIRestrict ,
  	...
  	@VoiceMessage as VoiceMessage, @NewMessage as NewMessage,  @PINChangeOpt as PINChangeOpt, @FreeCallNo as FreeCallNo ;
