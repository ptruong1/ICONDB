...
-- =============================================
CREATE PROCEDURE [dbo].[p_get_visit_videos_10262015] 
@facilityID int,
@fromDate varchar(10),
@toDate	  varchar(25),
@InmateID varchar(12),
...
AS
 ...
 Begin
	  Select distinct RoomID as recordID, [ApmDate]   ,LEFT(ApmTime,8) as ApmTime, A.LimitTime, A.VisitorID, D.Email, A.EndUserID, A.Status, A.InmateID, A.InmateName, D.[VLastName], D.[VFirstName],
  ...
  FROM [leg_Icon_dev].[dbo].[tblVisitEnduserSchedule] A 
  left join tblVisitLocation L on A.locationID = L.LocationID 
  Left Join [leg_Icon_dev].[dbo].[tblVisitType] X on A.visitType = X.VisitTypeID
  left Join [leg_Icon_dev].[dbo].[tblInmate] B
  On A.facilityID = B.FacilityID and A.InmateID = B.InmateID
    left Join  [leg_Icon_dev].[dbo].tblVisitors D
	    On  A.VisitorID = D.VisitorID
	Left Join  [leg_Icon_dev].[dbo].tblVisitPhoneServer C
	    On A.FacilityID = C.FacilityID 
	Left Join  [leg_Icon_dev].[dbo].tblVisitStatus Y
 ...
 If @InmateID <> '' -- Level 1
  ...
  FROM [leg_Icon_dev].[dbo].[tblVisitEnduserSchedule] A 
  left join tblVisitLocation L on A.locationID = L.LocationID 
  Left Join [leg_Icon_dev].[dbo].[tblVisitType] X 
...
, A.StationID, L.LocationName
  ...
  left Join  [leg_Icon_dev].[dbo].tblVisitors D
	...
	Select distinct  RoomID as recordID, [ApmDate]   ,[ApmTime], A.LimitTime, A.VisitorID, D.Email, A.EndUserID, A.Status, A.InmateID, A.InmateName, D.[VLastName], D.[VFirstName],
  B.PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	   A.VisitType, X.Descript, InmateLogInID, VisitorLogInID, Note, isNull(AlertCellPhone,'') as AlertCellPhone,  
	  isNull(AlertCellCarrier,'') as AlertCellCarrier, Y.Descript as ApptStatus, D.Phone1, ISNULL(TotalCharge,0) as TotalCharge, A.RecordOpt, ISNULL(A.RecordStatus,0) as RecordStatus, C.ChatServerIP as ChatServerIP
, A.StationID, L.LocationName
