...
-- =============================================
CREATE PROCEDURE [dbo].[p_Inmate_login_from_kiosk_v5] 
...
BEGIN
   ...
   Declare  @MedicalKiteForm bit, @InmateKiteForm bit, @GrievanceForm bit,@facilityID int, @locationID int,@roomID int,@visitType tinyint, @InmateLocationID int, 
   ...
   Declare @VisitorName varchar(50),	@InmateName  varchar(50) ,	@AppDateTime	datetime ,	@AppDuration int ,	@SessionID	varchar(20)  ,@LocationName varchar(30), @FormTemplate int,
   ...
   Declare  @GrievanceTemplate int, @InmateTemplate int, @MedTemplate int, @LegalTemplate int ;
   SET @facilityID =0 ;
   --SET @inmateID ='0';
   SET @SoftphoneOpt =0;
   SET @FormOpt = 0 ;
   ...
   SET @locationName ='';
   SET @GrievanceTemplate  =1
   SET @InmateTemplate =1;
   SET @MedTemplate =1;
   SET @LegalTemplate =1;
   select @facilityID= a.FacilityID ,@locationID  =a.LocationID,@LocationName= b.LocationName  FROM tblVisitPhone  a with(nolock) join tblVisitLocation b with(nolock)  on (a.FacilityID=b.FacilityID and a.LocationID =b.LocationID )
   ...
   If(@facilityID> 0)
    begin
		select @InmateName = FirstName + ' ' + LastName , @DOB =isnull(DOB,'')  from tblInmate with(nolock)  where FacilityId = @FacilityId and PIN =@PIN and [Status]=1;
		SELECT @InmateSO =bookingNo from tblInmateInfo with(nolock)  where FacilityID =@facilityID and PIN=@PIN and InmateID=@inmateID 
		...
		if(@inmateID <> '0')
		 begin
			...
			if(@InmateLocationID is null or @InmateLocationID =0)
			 ...
			 begin
				if((@facilityID in ( 578,478, 866) ) and (@locationID <> @InmateLocationID) )
				 begin
					SET @inmateID ='0';
					SET @InmateName='';
					SET @InmateSO ='';
		...
		if(@FormOpt =1)
				select @MedicalKiteForm=MedicalKite, @InmateKiteForm = InmateKite,@GrievanceForm= Grievance ,@MedicalKiteReceiveEmail=MedicalKiteReceiveEmail, @InmateKiteReceiveEmail=InmateKiteReceiveEmail, @GrievanceReceiveEmail=GrievanceReceiveEmail, @MedProvider= isnull(MedProvider ,'Affordable care Act'),
				 @GrievanceTemplate =isnull(GrievanceTemplate,1), @InmateTemplate = isnull(InmateTemplate,1), @MedTemplate =isnull( MedTemplate,1), @LegalTemplate = isnull(LegalTemplate,1)  from tblfacilityForms where facilityID = @facilityID ;
    end
    ...
    @FormOpt as  FormOpt, @MedicalKiteForm as MedicalKiteForm, @InmateKiteForm  as InmateKiteForm , @GrievanceForm as  GrievanceForm, @MedicalKiteReceiveEmail as  MedicalKiteReceiveEmail , @InmateKiteReceiveEmail as InmateKiteReceiveEmail, @GrievanceReceiveEmail as  GrievanceReceiveEmail, @timeDiff as TimeDiff, @MedProvider as  MedProvider, @FacilityName as FacilityName, @locationName  as LocationName,
	 @GrievanceTemplate as GrievanceTemplate,  @InmateTemplate as InmateTemplate, @MedTemplate  as MedTemplate, @LegalTemplate as LegalTemplate;
