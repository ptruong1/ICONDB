...
CREATE PROCEDURE [dbo].[p_create_user6]
@UserID varchar(20),  
@Password  varchar(20),
@FacilityID int ,
@AgentID  int,
...
Declare  @return_value int, @nextID int, @CurrentID int, @tblAuthUsers nvarchar(32) ;
	   ...
	   insert tblAuthUsers(AuthID, FacilityConfig, UserControl, PhoneConfig, CallControl ,DebitCard ,InmateProfile ,Report, CallMonitor ,Messaging, VideoVisit, ServiceRequest,MyReport,[Admin],PowerUser,[Finance-Auditor],Investigator,DataEntry,UserDefine)
		   values(@CurrentID, @FacilityConfig, @UserControl, @PhoneConfig, @CallControl ,@DebitCard ,@InmateProfile ,@Report, @CallMonitor ,@Messaging,@VideoVisit, @ServiceRequest,0,@Admin,@PowerUser,@Finance_Auditor,@Investigator,@DataEntry,@UserDefine);
...
values(@UserID, @password  , @FacilityID , @AgentID , @LastName  ,@FirstName   ,@MidName  , @ID, @Department  , @nextID, @Phone  ,@Email,  @userLevel ,@Createby,@IPaddress);
