CREATE PROCEDURE [dbo].[p_get_Messages2]  -- CAll by ICON
...
If(@InmateID <>'' and @fromDate <> '' and @toDate <>'')
	select t1.FacilityID, t1.MailboxID  ,MessageID, t1.InmateID, FirstName, LastName , t1.status, MessengerNo as [Caller ID], MessengerNo as [Caller Account],MessageDate ,MessageName, t2.folderDate 
...
Else  If(@InmateID <>''  and @toDate <>'')
	...
	order by   t1.MailboxID  ,MessageID
Else If(@InmateID ='' and @fromDate <> '' and @toDate <>'')
select t1.FacilityID, t1.MailboxID  ,MessageID, t1.InmateID, FirstName, LastName , t1.status, MessengerNo as [Caller ID], MessengerNo as [Caller Account],MessageDate ,MessageName, t2.folderDate 
	from tblMailBox  t1  with(nolock) , tblMailboxDetail t2  with(nolock), tblInmate  t3  with(nolock)
