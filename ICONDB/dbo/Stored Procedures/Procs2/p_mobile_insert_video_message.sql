CREATE PROCEDURE [dbo].[p_mobile_insert_video_message]  -- Family send Email to Inmate Email Box
...
AS
--Begin
--	Declare  @messageID int,  @folderDate char(8),  @message VARCHAR(50), @InmateMailboxID as int, @FFMailBoxID int, @VideoMessageAppdReq tinyint,@MessageStatus tinyint, @timezone smallint, @localTime datetime;
...
--	-- set video status =2
--	INSERT tblMailboxDetail(MailBoxID ,  messageID,  MessengerNo, MessageDate , MessageName ,Message   ,  IsNew,  MessageTypeID,SenderMailBoxID, Charge ,MessageStatus, videostatus)
--				values(@MailBoxID, @messageID, @AccountNo,  @LocalTime,@Subject, @message + '.mp4', 1,4,  @FFMailBoxID,@Charge,@MessageStatus,2);
...
Begin
	...
	SET @LocalTime = DATEADD (HOUR, @timeZone , GETDATE())
	-- set video status =2
	INSERT tblMailboxDetail(MailBoxID ,  messageID,  MessengerNo, MessageDate , MessageName ,Message   ,  IsNew,  MessageTypeID,SenderMailBoxID, Charge ,MessageStatus, videostatus, MonitorOpt)
				values(@MailBoxID, @messageID, @AccountNo,  @LocalTime,@Subject, @Subject, 1,4,  @FFMailBoxID,@Charge,@MessageStatus,2, @MonOpt);
