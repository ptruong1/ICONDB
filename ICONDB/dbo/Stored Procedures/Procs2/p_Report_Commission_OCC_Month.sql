CREATE PROCEDURE [dbo].[p_Report_Commission_OCC_Month]
...
If(@AgentID	>1 AND @facilityID =0 )
	...
	(Select   Location, FromNo, DATEPART(dd,tblcallsbilled.RecordDate) as D, Count(calldate) as CallsCount 
...
Else
	...
	(Select   tblcallsbilled.FacilityID, FromNo, DATEPART(dd,tblcallsbilled.RecordDate) as D, Count(calldate) as CallsCount 
			...
			from tblcallsbilled with(nolock),  tblCalltype  with(nolock), tblBilltype  with(nolock)  , tblCommrate with(nolock)  WHERE
						...
						--tblcallsbilled.agentID = @agentID and
								(RecordDate between @fromDate and dateadd(d,1,@toDate) )   and convert (int,duration ) >15
								and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
			group by tblcallsbilled.FacilityID, FromNo, DATEPART(dd,RecordDate)) as X
