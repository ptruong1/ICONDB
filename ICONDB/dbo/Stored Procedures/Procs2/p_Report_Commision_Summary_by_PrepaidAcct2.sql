CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_PrepaidAcct2]
@facilityID	int,
@AgentID	int,
 ...
 Begin
	If(@AccountNo <> '')
	    If @divisionID > 0
			...
			select   tblcallsBilled.billtono as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue,
			 Sum(Pif) as Pif,Rate,  
			...
			from tblcallsbilled with(nolock), tblCommRateAgent with(nolock), tblPrepaid with(nolock), tblBadDebt with(nolock)  where  
								...
								tblPrepaid.PhoneNo = @AccountNo and
