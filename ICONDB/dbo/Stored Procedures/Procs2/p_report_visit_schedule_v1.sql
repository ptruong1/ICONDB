...
-- =============================================
CREATE PROCEDURE [dbo].[p_report_visit_schedule_v1] 
@FacilityID int,
@Status tinyint
AS
BEGIN
	if(@status = 1 or @status = 2)
	 begin
		select  a.ApmNo as VisitConfirmNumber, IsNULL( b.EntityID,'NA')  EntityID, b.vlastName as lastName, b.vfirstname Firstname, b.phone1 as PhoneNo, isnull(DOB,'NA') DOB  ,   CASE c.VisitRemain When -1 then 0 else c.VisitRemain end VisitBalance , DATEADD (MINUTE,datepart(MINUTE, Apmtime),  DATEADD(hour,  datepart(hour, Apmtime),ApmDate)) ApmDateTime, a.InmateID
