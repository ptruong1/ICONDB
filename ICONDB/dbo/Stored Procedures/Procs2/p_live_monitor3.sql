...
CREATE PROCEDURE p_live_monitor3
@facilityID	int,
@DivisionID       int,
@locationID	int,
...
If(@stationID <> '')
	...
	select isnull(RecordID,0) RecordID, (CASE Isnull(ToNo,'') When '' then 'Inactive' Else 'Active' End) Status,  	Isnull(PIN,0) as PIN, StationID, isnull(Tono,'') Tono , isnull(CAST(Recorddate AS Varchar(30)),'') ConnectDateTime ,
	 isnull(billtype,'') Billtype,	 Isnull(toCity,'') ToCity, isnull(ToState,'') toState,  Channel,( select P.ComputerName from tblACPs P  with(nolock)  where O.UserName=P.IPAddress) ACP,
