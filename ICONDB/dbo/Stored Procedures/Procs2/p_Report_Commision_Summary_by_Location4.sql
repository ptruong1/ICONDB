CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_Location4]
@facilityID	int,
@AgentID	int,
...
Else
   Begin
	...
	Else
		...
		from
		(select  tblFacilityLocation.Descript as Location, tblcallsbilled.calldate, 1 CallCount, (dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, (CallRevenue) CallRevenue, 0 as Pif, 0 BadDebtRate,  
			0 as BadDebt,  (tblCommrate.CommRate * 100) as CommRate ,  
			CAST (((CallRevenue)   * tblCommrate.CommRate) as Numeric(12,4))      as CommPaid
			from tblcallsbilled with(nolock), tblcommrate with(nolock),  tblFacilityLocation  with(nolock) , tblANIs  with(nolock) where 
		...
		group by X.Location, X.Commrate
		Order by X.Location, X.Commrate
