...
-- =============================================
CREATE PROCEDURE [dbo].[p_select_available_appointment_140604_old]
	@facilityID	int,
	@locationID int,
	@InmateID	varchar(12),
...
BEGIN
    ...
    Select @hourBeforeVisit = isnull(HourBeforeVisitOnSite,0),@ApmIncrement= isnull(Increment,0),@MaxFTime=isnull(MaxDuration,0),@InterApm = isnull(InterSection,0)    from tblVisitfacilityConfig where FacilityID =@facilityID ;
	...
	if(select COUNT(*) from tblVisitLocationSchedule where FacilityID =@facilityID and LocationID =@locationID) >0
	 begin
		...
		SELECT @period = isnull(LimitTime,15), @InterApm= InterApm  , @FromTime1= fromTime1, @toTime1 = totime1, @FromTime2= FromTime2,@toTime2=toTime2,
		 @FromTime3= fromTime3, @toTime3 = totime3 from [leg_Icon].[dbo].[tblVisitLocationSchedule]
		    where FacilityID = @facilityID and LocationID =@locationID and scheduleday= @dw and visitType =2;
	 end 
	else
		SELECT @period = isnull(LimitTime,15), @InterApm= InterApm  , @FromTime1= fromTime1, @toTime1 = totime1, @FromTime2= FromTime2,@toTime2=toTime2 from [leg_Icon].[dbo].[tblVisitFacilitySchedule]
