CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_DebitNo2]
@facilityID	int,
@AgentID	int,
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) > 0
 Begin
	If(@AccountNo <> '')
	    If @divisionID > 0
		select  tblcallsBilled.CreditcardNo as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,Rate,  
		...
		group by  tblcallsBilled.CreditcardNo, tblCommRateAgent.CommRate,Rate, PifPaid  
		Order by  tblcallsBilled.CreditcardNo, tblCommRateAgent.CommRate,Rate, PifPaid  
	    else
		select  tblcallsBilled.CreditcardNo as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,Rate,  
		CAST ((sum(CallRevenue) * isnull( Rate,0)) as numeric(7,2))  as BadDebt, (tblCommrateAgent.CommRate * 100) as CommRate ,  
		(CASE PifPaid  when 0 then CAST (((sum(CallRevenue) - sum(CallRevenue) * isnull( Rate,0) )   * tblCommrateAgent.CommRate)  as numeric(12,4) )  
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) = 0
 Begin
	If(@AccountNo <> '')
	    If @divisionID > 0
		select  tblcallsBilled.CreditcardNo as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,'0' BadDebtRate, 0 as Rate, 
		...
		from tblcallsbilled with(nolock), tblCommRateAgent with(nolock) where  
							 tblcallsBilled.errorcode = '0' and 
							tblcallsbilled.AgentID = tblCommrateAgent.AgentID AND
							...
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
		group by  tblcallsBilled.CreditcardNo, tblCommRateAgent.CommRate,BadDebtRate, PifPaid  
		Order by  tblcallsBilled.CreditcardNo, tblCommRateAgent.CommRate,BadDebtRate, PifPaid  
	    else
		select  tblcallsBilled.CreditcardNo as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,'0' BadDebtRate, 0 as Rate, 
		'0'  as BadDebt,  (tblCommrateAgent.CommRate * 100) as CommRate ,  
		CAST ((sum(CallRevenue)   * tblCommrateAgent.CommRate) as Numeric(12,4))      as CommPaid
...
If (@facilityID > 0)
 Begin
	If(@AccountNo <> '')
		...
		select  tblcallsBilled.CreditcardNo as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, 0 as Pif,0 BadDebtRate,  
		...
		from tblcallsbilled with(nolock), tblCommRate with(nolock) where  
							...
							--tblcallsbilled.agentID = @agentID and
							(RecordDate between @fromDate and dateadd(d,1,@todate) ) and  tblcallsBilled.CreditcardNo	= @AccountNo and convert (int,duration ) >5 
		group by  tblcallsBilled.CreditcardNo, tblCommRate.CommRate,BadDebtRate, PifPaid  
		Order by  tblcallsBilled.CreditcardNo, tblCommRate.CommRate,BadDebtRate, PifPaid  
	
	Else
		
		select  tblcallsBilled.CreditcardNo as AccountNo,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,BadDebtRate,  
