CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_Division]
@facilityID	int,
@AgentID	int,
...
If (@AgentID >1 AND @facilityID =0 )
 Begin
	If(@divisionID >0)
		...
		select  tblFacilityDivision.DepartmentName as DiVision, tblcallsbilled.calldate, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(12,4)) as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif, isnull(Rate,0) BadDebtRate,  
		CAST ((sum(CallRevenue) *  isnull( Rate,0)) as numeric(12,5))  as BadDebt,  (tblCommrateAgent.CommRate * 100) as CommRate ,  
		(CASE PifPaid  when 0 then CAST (((sum(CallRevenue) - sum(CallRevenue) *  isnull( Rate,0) )   * tblCommrateAgent.CommRate)  as numeric(12,4) )  
				      Else   CAST (((sum(CallRevenue) - sum(CallRevenue) *  isnull( Rate,0)  - sum(Pif) - Sum(NIF))   *tblCommrateAgent.CommRate)  as numeric(12,5) )  + sum(Pif)    end) as CommPaid
