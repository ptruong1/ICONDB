...
-- =============================================
CREATE PROCEDURE [dbo].[p_live_video_monitor_1107]
@facilityID	int
AS
...
Select RoomID as recordID, [ApmDate]   ,(CONVERT(varchar(15),CAST(ApmTime AS Time),100)) as apmTime, A.LimitTime, A.VisitorID, D.Email, A.EndUserID, A.Status, A.InmateID, A.InmateName, D.[VLastName], D.[VFirstName],
  B.PIN  , ISNULL(VisitDuration,0) as duration , A.Relationship,
	  ...
	  isNull(AlertCellCarrier,'') as AlertCellCarrier, Y.Descript as ApptStatus, D.Phone1, ISNULL(TotalCharge,0) as TotalCharge, D.RecordOpt, ISNULL(A.RecordStatus,0) as RecordStatus, C.ChatServerIP as ChatServerIP
