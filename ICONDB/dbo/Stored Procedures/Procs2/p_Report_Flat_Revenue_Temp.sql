CREATE PROCEDURE [dbo].[p_Report_Flat_Revenue_Temp] 
(
	@AgentID	int,
	@DivisionID	int
)
AS
	...
	Status = CASE IsNull(X.MostRecentServiceMonth,0) WHEN 0 THEN 'Phone is not active in last 6 months' ELSE '' END
		...
		(select MAX(RecordDate)  from tblCallsBilledArchive where tblANIs.ANiNo = tblCallsBilledArchive.FromNo)  AS "MostRecentServiceMonth"
