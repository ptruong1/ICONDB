CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_By_CallType5]
	...
	If (select count(*) from tblBadDebt  with(nolock) where AgentID = @AgentID) > 0
	   if @divisionID > 0
		if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
			Select CallType, Division, sum(CallCount) as Callcount, sum(CallDuration) as CallDuration, Sum(BadDebt) as Baddebt, sum(Pif) as Pif, Sum(CommPaid) as CommPaid, sum(CallRevenue) as CallRevenue 
			from
			...
			group by Division, CallType
			Order by  Division, CallType
		else
			Select CallType, Division, sum(CallCount) as Callcount, sum(CallDuration) as CallDuration, Sum(BadDebt) as Baddebt, sum(Pif) as Pif, Sum(CommPaid) as CommPaid, sum(CallRevenue) as CallRevenue 
			from
			(select Calldate, Location as Division,fromno,tono, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype , RecordDate, 1  as CallCount,  dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement )  as CallDuration,  CallRevenue, Pif,  (isnull(Rate,0)*100) BadDebtRate,  
...
Else
	...
	Select CallType, sum(CallCount) as Callcount, sum(CallDuration) as CallDuration, Sum(BadDebt) as Baddebt, sum(Pif) as Pif, Sum(CommPaid) as CommPaid, sum(CallRevenue) as CallRevenue 
			from
