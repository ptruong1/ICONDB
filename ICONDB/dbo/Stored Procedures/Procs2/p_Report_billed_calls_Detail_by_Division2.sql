CREATE PROCEDURE [dbo].[p_Report_billed_calls_Detail_by_Division2]
@FacilityID	int,
@AgentID	int,
...
Begin
	...
	if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
		Select    tblFacility.Location as Division ,fromNo, Tono,RecordDate,  tblBilltype.Descript  as  BillType ,
			 dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement )   as Duration,	   CallRevenue 
			  from tblcallsBilled  with(nolock), tblBilltype  with(nolock), tblANIs with(nolock) , tblFacility with(nolock), tblCommrateAgent with(nolock)
...
Begin
	...
	Select    tblFacilityDivision.DepartmentName as Division , fromNo, Tono,RecordDate,  tblBilltype.Descript  as  BillType ,
			  dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement )   as Duration,	   CallRevenue 
			  from tblcallsBilled  with(nolock), tblBilltype  with(nolock), tblANIs with(nolock) , tblFacilityDivision with(nolock), tblCommrateAgent with(nolock)
			   where  tblBilltype.Billtype = tblcallsBilled.billtype and
				   tblcallsBilled.errorcode = '0' and 
				   tblcallsBilled.FromNo = tblANIs.ANIno and
				   tblcallsbilled.AgentID = tblCommrateAgent.AgentID AND
