...
CREATE PROCEDURE [dbo].[p_Report_Commision_Summary1]
@facilityID	int,
@AgentID	int,
...
If( @AgentID >1  and @facilityID =0 ) 
 Begin
	If (select count(*) from tblBadDebt  with(nolock) where AgentID = @AgentID) > 0
		select Calldate, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(12,4)) as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif, isnull(Rate,0)  Rate ,  
			...
			(CASE PifPaid  when 0 then CAST (((sum(CallRevenue) - sum(CallRevenue) * isnull( Rate,0) )   * tblCommRateAgent.CommRate)  as numeric(12,4) )  
					      Else   CAST (((sum(CallRevenue) - sum(CallRevenue) * isnull(Rate,0)  - sum(Pif)   -  sum(isnull(Nif,0)))   * tblCommRateAgent.CommRate)  as numeric(12,4) )  + sum(Pif)    end) as CommPaid
		...
		select Calldate, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(9,4)) as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif, 0 Rate ,  
			0 as BadDebt,  (tblCommRateAgent.CommRate * 100) as CommRate ,  
			(CASE PifPaid  when 0 then CAST (((sum(CallRevenue) )   * tblCommRateAgent.CommRate)  as numeric(7,2) )  
					      Else   CAST (((sum(CallRevenue) - sum(Pif)   -  sum(isnull(Nif,0)))   * tblCommRateAgent.CommRate)  as numeric(12,4) )  + sum(Pif)    end) as CommPaid
...
Else
 ...
 Begin
	...
	select Calldate, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(12,4)) as CallDuration, sum(CallRevenue) CallRevenue,  
