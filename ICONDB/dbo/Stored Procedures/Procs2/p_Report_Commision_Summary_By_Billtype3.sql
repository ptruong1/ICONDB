CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_By_Billtype3]
@facilityID	int,
@AgentID	int,
	    ...
	    If @divisionID > 0
		  if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
			...
			from tblcallsbilled with(nolock),  tblBilltype with(nolock) , tblCommrate with(nolock) , tblBadDebt with(nolock) ,tblFacility with(nolock) where 
								...
								(RecordDate between @fromDate and dateadd(d,1,@todate) )  and 
								  convert (int,duration ) >5      and billtono not in (select authNo from  tblofficeANI with(nolock)  where  Billabe =0)
			group by  tblFacility.Location,  tblbilltype.Descript, tblCommrate.CommRate,Rate,PifPaid 
			Order by  tblFacility.Location, tblbilltype.Descript,tblCommrate.CommRate,Rate,PifPaid  
	    else --Division = 0
		   if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
			select tblFacility.Location, tblbilltype.Descript as Billtype,  count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,Rate,  
			CAST ((sum(CallRevenue) *  isnull( Rate,0)) as numeric(12,4))  as BadDebt, (tblCommrateAgent.CommRate * 100) as CommRate ,  
 ...
 Begin 
	...
	If(@billtype	<> '')
		...
		from tblcallsbilled with(nolock),  tblBilltype with(nolock) , tblCommrate with(nolock)  ,tblFacility with(nolock) where 
							...
							(RecordDate between @fromDate and dateadd(d,1,@todate) ) and  
							  convert (int,duration ) >5      and billtono not in (select authNo from  tblofficeANI with(nolock)  where  Billabe =0)
		group by tblFacility.Location,tblbilltype.Descript, tblCommrate.CommRate,BadDebtRate,PifPaid 
		Order by tblFacility.Location, tblbilltype.Descript,tblCommrate.CommRate,Rate,BadDebtRate,PifPaid 
	    else -- division = -1
	    if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
		select tblFacility.Location, tblbilltype.Descript as Billtype,  count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,'0' BadDebtRate, 0 as Rate,  
		'0'  as BadDebt,  (tblCommrateAgent.CommRate * 100) as CommRate ,  
...
If ( @facilityID > 0 )
 Begin 
	...
	select BillType
		...
		from
		(select tblbilltype.Descript as Billtype,  1 CallCount, (dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, (CallRevenue) CallRevenue, 0 as Pif, 0 BadDebtRate,  
		0  as BadDebt,  (tblCommrate.CommRate * 100) as CommRate ,  
		--(CASE PifPaid  when 0 then CAST (((sum(CallRevenue) - sum(CallRevenue) *  isnull( BadDebtRate,0) )   * tblCommrate.CommRate)  as numeric(7,2) )  
			--	      Else   CAST (((sum(CallRevenue) - sum(CallRevenue) *  isnull( BadDebtRate,0)  - sum(Pif) )   *tblCommrate.CommRate)  as numeric(7,2) )  + sum(Pif)    end) as CommPaid
		...
		group by X.BillType, X.Commrate
		Order by X.BillType, X.Commrate
