...
-- =============================================
CREATE PROCEDURE [dbo].[p_select_available_appointment_by_admin]
	@facilityID	int,
	@locationID int,
	@InmateID	varchar(12),
...
BEGIN
	...
	Declare @VisitPerday tinyint, @visitPerWeek tinyint, @VisitPerMonth tinyint,  @hourBeforeVisit tinyint,@ExtID varchar(15), @TimeBeforeApm tinyint;		
    ...
    Select @VisitPerday= VisitPerDay,@visitPerWeek= VisitPerWeek, @VisitPerMonth=VisitPerMonth, @hourBeforeVisit=  isnull(HourBeforeVisitOnSite,0)  from tblVisitFacilityConfig where FacilityID = @facilityID ;
	...
	*/
	...
	Else
	 begin 
		if(select COUNT(*) from tblVisitLocationSchedule with(nolock) where FacilityID =@facilityID and LocationID =@locationID) >0
		 begin
			--select 'TEST'
			SELECT @period = isnull(LimitTime,15), @InterApm= isnull(InterApm,0)  , @FromTime1= fromTime1, @toTime1 = totime1, @FromTime2= FromTime2,@toTime2=toTime2,
		 ...
		 begin
			SELECT @period = isnull(LimitTime,15), @InterApm= isnull(InterApm ,0),@TimeBeforeApm = isnull(TimeBeforeApm ,0) , @FromTime1= fromTime1, @toTime1 = totime1, @FromTime2= FromTime2,@toTime2=toTime2, @FromTime3= fromTime3, @toTime3 = totime3 
	...
	set @period = @period + @TimeBeforeApm; 
	--select @hourBeforeVisit, @period , @InterApm
	--Period 1
	if(@FromTime1 is not null)
	 begin
	...
	if(@CurrentDate < @scheduleDate)
		select  ScheduleTime,ApmTimeLimit as ApmTimeMin,  ApmTimeLimit, 1 as ApmIncrement, @recortOpt as RecordOpt  
