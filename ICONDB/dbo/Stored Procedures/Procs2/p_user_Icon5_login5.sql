...
CREATE PROCEDURE  [dbo].[p_user_Icon5_login5]
@userName  varchar(20),
@password	varchar(20),
...
Declare @loginID	bigint, @FacilityID int, @u varchar(20) , @p varchar(20), @agentID int , @userLevel   tinyint , @transcriptOpt	tinyint, @ActTime datetime, @FormsOpt tinyint,
	...
	Declare @return_value int, @nextID int, @ID int, @tblUserLogs nvarchar(32), @UserPhone varchar(10), @Email nvarchar(50), @SecondFactor bit  ;
SET @FacilityID = 0;
SET @agentID =0 ;
SET @userLevel  =0;
SET @transcriptOpt	= 0;
...
@authID=authID, @FirstName=FirstName, @LastName=LastName, @UserPhone=Phone, @Email=Email, @SecondFactor= isnull(SecondFactor, 0) 
 ...
 Begin
	...
	if (@agentID =1) 
		...
		select 1 as FacilityID, Company as  Location ,  Address1, City, StateCode as State, 
				...
				@LastName as LastName,@FormsOpt as FormsOpt, @UserPhone as UserPhone, @Email as Email, @SecondFactor as SecondFactor
...
Else 
 Begin
	...
	@transcriptOpt as  transcriptOpt,  @authID as authID ,@visitOpt as VisitOpt, @FirstName as FirstName, @LastName as LastName,@FormsOpt as FormsOpt, @UserPhone as UserPhone, @Email as Email, @SecondFactor as SecondFactor ;
  End
