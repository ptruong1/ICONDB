...
-- =============================================
CREATE PROCEDURE [dbo].[p_insert_visit_call_record_v2]
@ExtID varchar(10) ,
@PIN varchar(12), 
@FacilityID int ,
@CallDate	char(8), 
...
BEGIN
	...
	select  @MinuteCharge=PerMinCharge  , @ConnectFee=   ConnectFee from [leg_Icon].[dbo].tblVisitRate with(nolock) where RateID = @RateID and VisitType =@VisitType;
	Set  @RecordDate = dateadd(hh, @timeZone,getdate() );
    SET @CallRevenue = @ConnectFee + (@duration/60) * @MinuteCharge;
    insert [leg_Icon].[dbo].tblVisitCalls( ExtID, PIN , FacilityID , FolderDate, CallTime, RecordDate , duration , RateID,  ConnectFee ,MinuteCharge  ,
				CallRevenue, VisitType, VisitBilltype, RecordName,ServerIP,InmateID) 
