...
-- =============================================
CREATE PROCEDURE [dbo].[p_Report_outbound_terminate_V2]
	@AgentID int,
	@FromDate datetime,
	@ToDate datetime
AS
BEGIN
	...
	Insert  #tempTer ( OrigNumber ,TerNumber,ConnectDateTime,Calltype, Duration)
		   select fromno, tono, recordDate,isnull( Calltype, 'RL'), CEILING( isnull((OutDuration/60.0),1)) from tblcallsunbilled with(nolock) 
			where RecordDate >= @FromDate and RecordDate <= dateadd(day,1,@ToDate) and OutDuration >0   and FacilityID in (select FacilityID from tblfacility with(nolock) where AgentID = @AgentID); 

	Insert  #tempTer ( OrigNumber ,TerNumber,ConnectDateTime,Calltype, Duration)
		   select fromno, tono, recordDate,Calltype, CEILING(ACduration/60.00) from tblcallsbilled with(nolock) 
			where RecordDate >= @FromDate and RecordDate <= dateadd(day,1,@ToDate) and agentID =@AgentID; 
