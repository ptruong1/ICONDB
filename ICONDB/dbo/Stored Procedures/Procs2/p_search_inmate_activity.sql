...
-- =============================================
CREATE PROCEDURE [dbo].[p_search_inmate_activity]
@FacilityID int,
@InmateID	varchar(12),
		...
		if(@FirstName <> '' and @LastName <>''   and @FromDate <>'' and @todate<>'' )
		 begin
			...
			insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and inmateID in (select InmateID from  tblInmate with(nolock)  where facilityID =@facilityID and Firstname like @FirstName +'%'  AND lastName like @LastName +'%') 
				and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate);
			insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,30, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
			 ...
			 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select RecordName, b.InmateID, RecordDate, 3, stationID, '', duration, 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
		 ...
		 begin
			...
			insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			 ...
			 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
				...
				and RecordDate>=@FromDate and RecordDate <=dateadd(d,1,@toDate); 	   
			insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and inmateID in (select InmateID from  tblInmate with(nolock)  where facilityID =@facilityID   AND lastName like @LastName +'%') 
				and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate);
			insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,30, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
		 ...
		 begin
			...
			insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID 
				and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate);
			insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,30, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
 ...
 begin
	If(@InmateID <> '' and @FromDate <>'' and @todate<>'') 
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate);
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,30, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID and Apmdate >=@FromDate;
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID;
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and Apmdate >=@FromDate and Apmdate <=@toDate;
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName, ApmDate+ apmtime, 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and Apmdate >=@FromDate ;
	    insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,30, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, visitDuration, (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and  Apmdate <=@toDate;
...
Select RecordID , InmateID ,InmateName, ActivityDate, Description as ActivityType, isnull( StationID,'')  StationID, ContactPhone ,isnull( Duration,0) Duration, 
