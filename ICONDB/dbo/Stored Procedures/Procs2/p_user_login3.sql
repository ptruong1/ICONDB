...
CREATE PROCEDURE  [dbo].[p_user_login3]
@userName  varchar(20),
@password	varchar(20),
...
@transcriptOpt	tinyint, @IPaddressDB  varchar(16), @authID int, @EnCryptUser varbinary(200), @EnCryptPass varbinary(200)
SET @FacilityID = 0
SET @agentID =0 
SET @userLevel  =0
SET @transcriptOpt	= 0
...
Else IF   @userLevel = 2  --and  dbo.fn_comp_string ( @u, @userName)  =1  and dbo.fn_comp_string (  @p,  @password)  =1  
 Begin
	Insert tblUserLogs(UserName,Password,IPAddess,loginTime) 	Values(@userName,@password,@IPaddress,getdate())
	SET @loginID = @@identity 
	select 0 as FacilityID, Company as  Location ,  Address1, City, StateCode as State, Phone , ''  as Logo ,0  as PINRequired, AgentID,  @loginID as LoginID ,@userLevel as  userLevel ,@transcriptOpt as  transcriptOpt,  @authID as authID     from tblAgent with(nolock)
...
Else IF   @userLevel = 1 -- and  dbo.fn_comp_string ( @u, @userName)  =1  and dbo.fn_comp_string (  @p,  @password)  =1  
 Begin
	Insert tblUserLogs(UserName,Password,IPAddess,loginTime) 	Values(@userName,@password,@IPaddress,getdate())
	SET @loginID = @@identity 
	select 0 as FacilityID,'Legacy' as  Location ,'Valley View'  Address1,'Cypress'  City, 'CA' as State,'714-826-0547' Phone , ''  as Logo ,0  as PINRequired, '0' as  AgentID,  @loginID as LoginID ,@userLevel as  userLevel  ,@transcriptOpt as  transcriptOpt,  @authID as authID 
  End
