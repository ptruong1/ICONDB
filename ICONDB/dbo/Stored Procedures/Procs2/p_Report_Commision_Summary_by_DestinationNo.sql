CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_DestinationNo]
@facilityID	int,
@AgentID	int,
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) = 0
 Begin 
	If(@ToNo <> '')
		...
		select  ToNo, tblcallsbilled.calldate, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,5)))/60 as Numeric(12,5)) as CallDuration, sum(CAST(CallRevenue as numeric(9,4))) CallRevenue, Sum(Pif) as Pif,'0' BadDebtRate, 0 as Rate,
		...
		from tblcallsbilled with(nolock), tblCalltype with(nolock), tblBilltype  with(nolock) , tblCommRateAgent with(nolock)  where 
							...
							(RecordDate between @fromDate and dateadd(d,1,@todate) ) and ToNo = @ToNo  and
							 CAST (ResponseCode as int) < 100  and convert (int,duration ) >15 
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
		group by ToNo, tblcallsbilled.calldate,tblcalltype.Descript, tblBilltype.Descript,tblCommrateAgent.CommRate,BadDebtRate, PifPaid 
		Order by ToNo, tblcallsbilled.calldate,tblcalltype.Descript, tblBilltype.Descript,tblCommrateAgent.CommRate,BadDebtRate, PifPaid
