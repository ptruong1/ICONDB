CREATE PROCEDURE [dbo].[p_reply_email_message_v2]  -- Family send Email to Inmate Email Box
@MailBoxID bigint,
@MessageID	bigint,
...
Begin
	...
	INSERT tblMailboxDetail(MailBoxID ,  messageID,  MessengerNo, MessageDate , MessageName    ,  IsNew, [message], MessageTypeID,SenderMailBoxID, CCEmails , IsReply, MessageStatus, Charge, MonitorOpt, DeviceName )
				values(@SenderMailBoxID, @ReplyMessageID, @AccountNo, @LocalTime,@messageName,1, @Message,2,@MailBoxID,@CCEmails,0,@MessageStatus,@charge,@MonOpt, @DeviceName);
	--------------------
