...
-- =============================================
CREATE PROCEDURE [dbo].[p_select_available_appointment_1_test]
	@facilityID	int,
	@locationID int,
	@InmateID	varchar(12),
...
BEGIN
	...
	Declare @day tinyint, @dw tinyint, @NumOfStations smallint,@timezone smallint,@currentApmTemp smallint,@RecordOpt char(1),@Curenttime time(0),@SusStartDate date,@SusEndDate date;
	...
	--if(@locationID =0) 
	Select @locationID = locationID,@InmateLimitTime  =ISNULL(MaxVisitTime,@MaxFTime), @VisitPerDay = isnull(VisitPerDay,@VisitPerDay), 
			@VisitPerWeek=isnull(VisitPerWeek,@VisitPerWeek) ,@VisitPerMonth=isnull(VisitPerMonth,@VisitPerMonth) ,@ExtID = ExtID,
			@SusStartDate = SusStartDate, @SusEndDate = SusEndDate
	...
	if(@RelationshipID not in (0,8,7,9,10))
	 BEGIN
	    if( @SusStartDate is not null and @SusEndDate is not null and  @SusStartDate < @LegacyDate and  @legacyDate < @SusEndDate)
			goto SelectOUT;
	...
	Else
	 begin
	    SELECT @period = isnull(LimitTime,15), @InterApm= isnull(InterApm,@InterApm)  , @FromTime1= PriFromTime, @toTime1 = PriTotime from [leg_Icon].[dbo].[tblVisitFacilitySchedule] with(nolock) where facilityID = @facilityID and scheduleday= @dw and FvisitType =@VisitType
	
	 end
	if( @FromTime1 is null and @toTime1 is null) --- No special time set for special Group
	 begin
	...
	--Period 1
	-- Modify for Houston only that allow Bondman, Clergy, Attorney schedule any time
	...
	--if(@InterApm = 0 or @ApmIncrement =0) @period -@InterApm  ,@MinTimeVisit + @InterApm
	--	SET @MinTimeVisit = @MaxTimevisit; @MinTimeVisit
	select  ScheduleTime, @MinTimeVisit    as ApmTimeMin,  @MaxTimevisit -@InterApm  + (@ApmIncrement *2)  as ApmTimeLimit ,@ApmIncrement as ApmIncrement, @RecordOpt as  RecordOpt 
