...
CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_PrepaidAcct]
@facilityID	int,
@AgentID	int,
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) = 0
 Begin
	If(@AccountNo <> '')
		...
		select  tblPrepaid.PhoneNo as AccountNo,tblcallsbilled.calldate,count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(7,2)))/60 as Numeric(9,2)) as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,'0' BadDebtRate, 0 as Rate,  
...
Else
 Begin
	If(@AccountNo <> '')
		...
		select tblPrepaid.PhoneNo as AccountNo,tblcallsbilled.calldate,count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(7,2)))/60 as Numeric(9,2)) as CallDuration, sum(CallRevenue) CallRevenue, 0 as Pif,0 BadDebtRate,  
