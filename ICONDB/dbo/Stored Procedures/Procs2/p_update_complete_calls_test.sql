...
CREATE PROCEDURE [dbo].[p_update_complete_calls_test]
@projectcode	char(6),
@calldate	char(6),
@duration	int,
@billtype	char(2),
	 ...
	 Begin
		If(@billtype = '07')  -- Update balance
		  Begin
			...
			Update tbldebit SET balance = balance - @callCharge, modifydate = getdate() where AccountNo = @AcctNo	
		...
		Else If(@billtype = '10')  -- Update balance
		  Begin
			...
			Update tblPrepaid SET balance = balance - @callCharge, modifydate = getdate() , UserName ='Enduser' where PhoneNo = @AcctNo	
