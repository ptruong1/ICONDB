...
-- =============================================
CREATE PROCEDURE [dbo].[p_update_get_video_visit_status] 
	@ConfirmID bigint,
	@status tinyint,
...
BEGIN
	...
	SET NOCOUNT ON;
	Declare @UserID varchar(20), @GroupID varchar(6), @AppDuration smallint, @ChatServerIP varchar(60),@timeZone tinyint,@CurrentStatus tinyint,
	...
	if(@UserType =1)
	 begin	
		select @UserID=InmateID,@AppDuration=LimitTime,	
			...
			from leg_Icon.dbo.tblVisitEnduserSchedule where  RoomID = @ConfirmID and status in (2,3);
