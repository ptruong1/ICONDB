...
CREATE PROCEDURE [dbo].[p_Report_Unbilled_calls_by_Division]
@FacilityID	int,
...
@DivisionID int
 ...
 AS
If(@DivisionID > 0)
	select  tblFacilityDivision.DepartmentName  as Division ,RecordDate  as Calldate , tblBilltype.Descript as Billtype  ,tblErrortype.Descript as Reason
	 from tblcallsUnbilled with(nolock) , tblErrortype  with(nolock), tblFacilityDivision  with(nolock) , tblANIs with(nolock)  , tblBilltype with(nolock) 
		  where  tblFacilityDivision.DivisionID	= 	tblANIs.DivisionID and	 tblBilltype.Billtype = tblcallsUnbilled.billtype and 
			...
			Group by tblFacilityDivision.DepartmentName,RecordDate,  tblBilltype.Descript  ,tblErrortype.Descript
