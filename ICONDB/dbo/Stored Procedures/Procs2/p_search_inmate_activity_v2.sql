...
-- =============================================
CREATE PROCEDURE [dbo].[p_search_inmate_activity_v2]
@FacilityID int,
@InmateID	varchar(12),
...
BEGIN
	...
	SET NOCOUNT ON;
	Declare  @InmateAct table (RecordID varchar(34), InmateID varchar(12),InmateName varchar(50),  ActivityDate datetime, CommunityType tinyint, StationID varchar(25), ContactPhone varchar(18), ContactName  varchar(50), Duration smallint, CommStatus tinyint) ;
 ...
 begin
		if(@FirstName <> '' and @LastName <>''   and @FromDate <>'' and @todate<>'' )
		 begin
			...
			insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
				select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock) where inmateID in (select InmateID from  tblInmate with(nolock)  where facilityID =@facilityID and Firstname like @FirstName +'%'  AND lastName like @LastName +'%')
				  and facilityID = @facilityID and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate) and  RecordFile <>'NA'; 	   
			 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			...
			insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				...
				and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate) and RecordOpt='Y';
			insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			 ...
			 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				...
				and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate)  and RecordOpt='Y';	 
	     end 
		 ...
		 begin
			...
			insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
				select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where 
				   facilityID = @facilityID and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate) and RecordFile <>'NA'; 	   
			insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
		 ...
		 begin
			...
			insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID 
				and Apmdate >=@FromDate   and RecordOpt='Y';
			insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
					from tblMailBoxDetail a with(nolock)  , tblMailBox b with(nolock)  where a.mailboxID = b.MailboxID 
			 ...
			 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select RecordName, b.InmateID, RecordDate, 3, stationID, '',  isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
				where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  
				and RecordDate>=@FromDate  and RecordOpt='Y' ;	 
	     end  
		 else if(  @todate<>'' ) 
		  begin
			...
			insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
				select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where 
				   facilityID = @facilityID and RecordDate <= dateadd(d,1,@toDate) and RecordFile <>'NA'; 	   
			insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			...
			insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID,isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID 
				and  Apmdate <=dateadd(d,1,@toDate)  and RecordOpt='Y';
			insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
					from tblMailBoxDetail a with(nolock) , tblMailBox b with(nolock)  where a.mailboxID = b.MailboxID 
			 ...
			 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
				select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
				where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  
				and  RecordDate <= dateadd(d,1,@toDate)  and RecordOpt='Y';	 
	     end   
	end
else
 begin
	If(@InmateID <> '' and @FromDate <>'' and @todate<>'') 
	 begin
	    ...
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate) and RecordFile <>'NA'; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate)  and RecordOpt='Y';
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
		 ...
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  and b.InmateID= @InmateID and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate)  and RecordOpt='Y'; 	
	 
	 end
	else if (@InmateID <> '' and @FromDate <>'') 
	 begin
		insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone, Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordDate>=@FromDate  and RecordFile <>'NA'; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID and Apmdate >=@FromDate  and RecordOpt='Y';
		...
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		 ...
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  and b.InmateID= @InmateID and RecordDate>=@FromDate  and RecordOpt='Y' ; 	
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordFile <>'NA'  ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID  and RecordOpt='Y';
	    ...
	    insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		 ...
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b  with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID and b.InmateID= @InmateID  and RecordOpt='Y'; 	

	 end
	else if(@FromDate <>'' and @todate<>'') 
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and RecordDate>=@FromDate and RecordDate <=dateadd(d,1,@toDate) and RecordFile <>'NA'; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and Apmdate >=@FromDate and Apmdate <=@toDate  and RecordOpt='Y';
		...
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		 ...
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  and  RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate)  and RecordOpt='Y'; 	

	 end
	else if(@FromDate <>'') 
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0'and RecordDate>=@FromDate and RecordFile <>'NA' ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName, ApmDate+ apmtime, 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and Apmdate >=@FromDate  and RecordOpt='Y' ;
	    insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
		 ...
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  and RecordDate>=@FromDate   and RecordOpt='Y'; 	

	 end
    else if( @todate<>'') 
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and  RecordDate <=dateadd(d,1,@toDate) and RecordFile <>'NA'; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID,isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and  Apmdate <=@toDate  and RecordOpt='Y';
		...
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		 ...
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '',isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID   and RecordDate <= dateadd(d,1,@toDate)  and RecordOpt='Y'; 	

	 end
	else 
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone , Duration, CommStatus ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono, Duration,1 from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and RecordFile <>'NA' ;	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
		...
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), (case [status] when 5 then 1 else 0 end) from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID  and RecordOpt='Y' ;
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
				from tblMailBoxDetail a with(nolock) , tblMailBox b with(nolock) where a.mailboxID = b.MailboxID and b.facilityID =@facilityID ;
		 insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select RecordName, b.InmateID, RecordDate, 3, stationID, '', isnull(duration,0), 1   from tblVisitcalls a with(nolock) , tblInmate b with(nolock), tblVisitPhone c with(nolock)
			where a.FacilityID = b.facilityID and b.facilityID = c.FacilityID and a.PIN = b.PIN and a.ExtID = c.ExtID and a.facilityID =@facilityID  and RecordOpt='Y'  ; 	
...
Select RecordID , InmateID ,InmateName, ActivityDate, b.Description as ActivityType, isnull( StationID,'')  StationID, ContactPhone , isnull( ContactName, '')  ContactName ,  (CASE b.CommID when 5 then 'NA' else  [dbo].[fn_ConvertSecToMin] ( Duration) end)  Duration, 
	...
	Order by  ActivityDate, InmateName 	 Desc ;
