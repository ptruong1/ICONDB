CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_PIN2]
@facilityID	int,
@AgentID	int,
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) > 0
 Begin
	If(@PIN <> '')
	    If @divisionID > 0
		select  tblcallsbilled.PIN as PIN,
			 ...
			 (tblCommrateAgent.CommRate * 100) as CommRate ,  
				...
				from tblcallsbilled with(nolock), tblCommRateAgent with(nolock),  tblBadDebt with(nolock)  where 
							...
							tblcallsbilled.FacilityID = @divisionID and
							 (RecordDate between @fromDate and dateadd(d,1,@todate) ) and  tblcallsbilled.PIN	= @PIN and convert (int,duration ) >5  
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
				group by  tblcallsbilled.PIN, tblcallsbilled.calldate,tblCommRateAgent.CommRate,Rate, PifPaid  
				Order by  tblcallsbilled.PIN, tblcallsbilled.calldate,tblCommRateAgent.CommRate,Rate, PifPaid 
	    else
		select  tblcallsbilled.PIN as PIN,
			tblcallsbilled.calldate,
			count(CallRevenue ) CallCount, 
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) = 0
 Begin
	If(@PIN <> '')
	    If @divisionID > 0
		select  tblInmate.PIN as PIN,tblcallsbilled.calldate,count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif,'0' BadDebtRate, 0 as Rate,
...
If (@facilityID > 0)
 Begin
	If(@PIN <> '')
		...
		select  tblInmate.PIN as PIN, tblcallsbilled.calldate, count(CallRevenue ) CallCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement ))  as CallDuration, sum(CallRevenue) CallRevenue, 0  as Pif,0 BadDebtRate,  
		...
		from tblcallsbilled with(nolock), tblCommRate with(nolock), tblInmate with(nolock)  where 
							...
							tblcallsbilled.PIN	= @PIN and convert (int,duration ) >5 
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0)
