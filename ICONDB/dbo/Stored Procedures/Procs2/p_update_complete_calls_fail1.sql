...
CREATE PROCEDURE [dbo].[p_update_complete_calls_fail1]
@projectcode	char(6),
@calldate	char(6),
@duration	int,
@billtype	char(2)
...
If(select count(*) from tblOncalls with(nolock) where  calldate = @calldate and projectcode = @projectcode and duration is null  and Errorcode ='0')  >0    and @duration >10
...
Begin
	...
	If( @duration > 10)
	 Begin
		Update  tblOncalls  SET duration =  @duration , Dberror ='Y'  where  calldate = @calldate and projectcode = @projectcode  
		
		--select  @callCharge=isnull(  CallRevenue,0)  from  tblOncalls  with(nolock)  where  calldate = @calldate and projectcode = @projectcode  
