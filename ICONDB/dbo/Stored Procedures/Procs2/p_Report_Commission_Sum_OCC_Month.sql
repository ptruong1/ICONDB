CREATE PROCEDURE [dbo].[p_Report_Commission_Sum_OCC_Month]
@facilityID	int,
@AgentID	int,
...
Else
	...
	(Select   tblcallsbilled.FacilityID, DATEPART(dd,tblcallsbilled.RecordDate) as D, Count(calldate) as CallsCount 
			...
			from tblcallsbilled with(nolock),  tblCalltype  with(nolock), tblBilltype  with(nolock)  , tblCommrate with(nolock)  WHERE
						...
						--tblcallsbilled.agentID = @agentID and
								(RecordDate between @fromDate and dateadd(d,1,@toDate) ) and convert (int,duration ) >15
								and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
			group by tblcallsbilled.FacilityID, DATEPART(dd,RecordDate)) as X
