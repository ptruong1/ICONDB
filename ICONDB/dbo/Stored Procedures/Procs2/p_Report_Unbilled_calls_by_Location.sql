...
CREATE PROCEDURE [dbo].[p_Report_Unbilled_calls_by_Location]
@FacilityID	int,
...
@LocationID int
 ...
 AS
If (@LocationID >0)
	select  tblFacilityLocation.Descript as Location ,RecordDate  as Calldate, tblbilltype.Descript as Billtype ,tblErrortype.Descript   as Reason
	 from tblcallsUnbilled , tblErrortype , tblFacilityLocation  with(nolock) , tblANIs with(nolock) , tblBilltype with(nolock) 
		  where  tblFacilityLocation.LocationID	= 	tblANIs.LocationID and	tblBilltype.Billtype = tblcallsUnbilled.billtype and 
			...
			Group by tblFacilityLocation.Descript ,RecordDate,  tblbilltype.Descript  ,tblErrortype.Descript
