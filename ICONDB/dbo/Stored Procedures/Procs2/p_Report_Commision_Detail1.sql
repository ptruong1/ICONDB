CREATE PROCEDURE [dbo].[p_Report_Commision_Detail1]
@facilityID	int,
@AgentID	int,
...
If(@AgentID	>1 AND @facilityID =0 )
 Begin
	If (select count(*) from tblBadDebt  with(nolock) where AgentID = @AgentID) > 0
		select Calldate, fromno,tono, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype , RecordDate, CAST( CAST(duration as numeric(7,4))/60 as Numeric(9,4)) as CallDuration,  CallRevenue, Pif,  isnull(Rate,0) BadDebtRate,  
		...
		select Calldate, fromno,tono, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype , RecordDate, CAST( CAST(duration as numeric(7,4))/60 as Numeric(9,4)) as CallDuration,  CallRevenue, Pif, 0  BadDebtRate,  
...
Else
  Begin
	...
	select Calldate, fromno,tono, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype , RecordDate, CAST( CAST(duration as numeric(7,4))/60 as Numeric(9,4)) as CallDuration,  CallRevenue,0  Pif, 0  BadDebtRate,  
