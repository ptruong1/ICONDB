CREATE PROCEDURE [dbo].[p_Report_Calls_Attempt_detail_01182016]
@FacilityID	int,
@AgentID	int,
...
Begin
	...
	Insert   #tempCallAtempt610  (fromNo , toNo , ConnectDateTime , BillType  ,  Reason , Duration,CallRevenue  )
		...
		Select   fromNo , toNo,   RecordDate as  ConnectDateTime ,  tblBilltype.Descript  as  BillType , 'Completed' as  Reason,
			CAST(duration/60 as varchar(2)) + ':' +  CAST( duration%60 as varchar(2)) as Duration, CallRevenue  from tblcallsBilled with(nolock)  ,  tblBilltype with(nolock) ,tblANIs with(nolock)
