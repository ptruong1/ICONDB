...
-- =============================================
CREATE PROCEDURE [dbo].[p_monthly_revenue_commission_by_facility]
@YYMM char(4)	
AS
BEGIN
	Set Nocount On;
...
--Replenish Fee
 ...
 select a.FacilityID, a.location, a.Revenue,isnull(b.SetUpfee,0) as SetUpfee,isnull(c.ReplenishFee,0) as ReplenishFee, isnull(d.CreditCardFee,0) as CreditCardFee, (f.CallRevenue * isnull(f.CommRate,0)) Commission   
    from @Revenue a 
		 left join @SetupFee b on a.facilityID = b.facilityID
		 left join @ReplenishFee c on a.facilityID = c.facilityID
		 left join @CreditCardFee d on a.facilityID = d.facilityID
		 left join @Commision f on a.facilityID = f.facilityID
