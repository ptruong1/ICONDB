...
CREATE PROCEDURE [dbo].[p_login_prepaid_Account1]
	...
	If (select count(*)  from  tblEndUsers with(nolock)   where  UserName =@EndUserUserName  ) = 1
	  Begin
		...
		If( @EnduserPassword =  @defaultPassword    )
		 Begin
			select  PhoneNo ,  FirstName , LastName  , Address ,  City,State, ZipCode ,  Balance ,  status, ContactPhone  from   tblPrepaid with(nolock) ,   tblEndUsers  with(nolock)
...
Else
 Begin
	...
	select FacilityID ,  PhoneNo ,  FirstName , LastName  , Address ,  City,State, ZipCode ,  Balance ,  status, ContactPhone  from   tblPrepaid with(nolock) ,   tblEndUsers  with(nolock)
