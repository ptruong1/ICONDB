...
CREATE PROCEDURE [dbo].[p_Insert_WatchListDetails_V2]
(	
	@FacilityID int,
	@watchListID int,
	@LocationID int,
...
AS
	...
	BEGIN
		DECLARE @count2 int;
		SELECT @count2 = COUNT(DNI) FROM [tblWatchList] WHERE (tblWatchList.DNI = @DNI AND tblWatchList.watchListID = @watchListID)
		IF @count2 > 0 --check to see if DNI is already in the watchlist
			RETURN -2;
		ELSE
		BEGIN
			EXEC   @return_value = p_create_nextID 'tblWatchList', @nextID   OUTPUT
	...
	BEGIN
			DECLARE @count3 int;
			SELECT @count3 = COUNT(PIN) FROM [tblWatchList] WHERE (tblWatchList.PIN = @PIN AND tblWatchList.watchListID = @watchListID)
	...
	BEGIN
		DECLARE @count4 int;
		SELECT @count4 = COUNT(InmateId) FROM [tblWatchList] WHERE (tblWatchList.InmateId = @InmateId AND tblWatchList.watchListID = @watchListID)
		...
		BEGIN
			...
			--INSERT INTO [tblWatchList] ([watchListID], LocationID, DivisionID,[ANI], [DNI], [PIN], [modifyDate], [Username], [WatchByID],[StationID], [ThirdPartyAlert],[LocationTrace])
			--		 VALUES (@watchListID, @LocationID, @DivisionID,@ANI, @DNI, @PIN, getdate(), @Username, @WatchByID,@StationID,@ThirdPartyAlert,@LocationTrace)
			Set @UserAction = 'Insert Watch List Detail: ' + @WatchListName + ' - InmateID: ' + CAST (@locationID as varchar(15))
					EXEC  INSERT_ActivityLogs5   @FacilityID,12, @UserAction, @Username, @UserIP
			RETURN 0;
		END
	END
	...
	BEGIN
		DECLARE @count5 int;
		SELECT @count5 = COUNT(DivisionID) FROM [tblWatchList] WHERE (tblWatchList.LocationID = @LocationID AND tblWatchList.watchListID = @watchListID and tblWatchList.LocationID = @LocationID)
	...
	BEGIN
		DECLARE @count6 int;
		SELECT @count6 = COUNT(DivisionID) FROM [tblWatchList] WHERE (tblWatchList.DivisionID = @DivisionID AND tblWatchList.watchListID = @watchListID and tblWatchList.LocationID = @LocationID)
		IF @count5 > 0 --check to see if pin is already in the watchlist
			RETURN -2;
		ELSE
		BEGIN
	...
	BEGIN
		DECLARE @count7 int;
		SELECT @count7 = COUNT(DivisionID) FROM [tblWatchList] WHERE (tblWatchList.EmailInmateId = @InmateId AND tblWatchList.watchListID = @watchListID and tblWatchList.LocationID = @LocationID)
		IF @count5 > 0 --check to see if pin is already in the watchlist
			RETURN -2;
		ELSE
		BEGIN
			EXEC   @return_value = p_create_nextID 'tblWatchList', @nextID   OUTPUT
                set           @ID = @nextID ;
			INSERT INTO [tblWatchList] ([ID], [watchListID], LocationID, DivisionID,[ANI], [DNI], [PIN], EmailInmateID, [modifyDate], [Username], [WatchByID],[StationID], [ThirdPartyAlert],[LocationTrace])
					 VALUES (@ID, @watchListID, @LocationID, @DivisionID,@ANI, @DNI, @PIN, @InmateID, getdate(), @Username, @WatchByID,@StationID,@ThirdPartyAlert,@LocationTrace)

			--INSERT INTO [tblWatchList] ([watchListID], LocationID, DivisionID,[ANI], [DNI], [PIN], [modifyDate], [Username], [WatchByID],[StationID], [ThirdPartyAlert],[LocationTrace])
			--		 VALUES (@watchListID, @LocationID, @DivisionID,@ANI, @DNI, @PIN, getdate(), @Username, @WatchByID,@StationID,@ThirdPartyAlert,@LocationTrace)

				Set @UserAction = 'Insert Watch List Detail: ' + @WatchListName + ' - EmailInmateId: ' + CAST (@InmateId as varchar(15)) 
					EXEC  INSERT_ActivityLogs5   @FacilityID,12, @UserAction, @Username, @UserIP
			RETURN 0;
		END
	END
