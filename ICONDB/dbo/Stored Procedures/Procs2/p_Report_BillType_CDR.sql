...
CREATE PROCEDURE [dbo].[p_Report_BillType_CDR]
@FacilityID	int,
@divisionID	int,
@locationID	int,
@phoneID	varchar(100),
...
begin
	If(@PIN <> '0' and  @callingCard <> ''  and  @DNI <> '' ) 
	 Begin
		...
		Insert   #tempCDR ( PhoneID , toNo , ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType  ,  State , Duration,CallRevenue  )
		Select  left( tblANIs.StationID,10)  PhoneID, toNo,   RecordDate as  ConnectDateTime ,left( tblcallsBilled.CreditcardNo,12) DebitCardNo ,  tblcallsBilled.PIN, LastName ,  FirstName,   tblBilltype.Descript  as  BillType , 'Completed' as  State,
			dbo.fn_ConvertSecToMin( duration) as Duration, CallRevenue  from tblcallsBilled with(nolock)  ,  tblBilltype with(nolock) ,tblInmate with(nolock) ,tblANIs with(nolock) 
		...
		Select  left( tblANIs.StationID,10) PhoneID , toNo,   RecordDate as  ConnectDateTime , DebitCardNo,  tblcallsUnbilled.PIN, LastName ,  FirstName,   tblBilltype.Descript  as  BillType  ,tblErrortype.Descript as  State,
			 '00:01:00'  as Duration,0  from tblcallsUnbilled with(nolock)  ,  tblBilltype with(nolock) ,tblInmate with(nolock) ,tblANIs with(nolock) , tblErrortype  with(nolock)
		 ...
		 select  left( tblANIs.StationID,10)  PhoneID , 'CTS'  as Tono ,      ReportTime   as ConnectDateTime   ,''  DebitCardNo,  tblincidentreport.PIN, LastName ,  FirstName,   'CTS'  as BillType ,    'Call To Staff'      as     state     ,
			  '00:01:00' as Duration, 0  as  CallRevenue  from tblincidentreport with(nolock) , tblInmate with(nolock)  ,tblANIs with(nolock) 
		...
		select  PhoneID , toNo , ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType  ,  State , Duration,CallRevenue  from  #tempCDR   WHERE   RecordID >= @startRowIndex
