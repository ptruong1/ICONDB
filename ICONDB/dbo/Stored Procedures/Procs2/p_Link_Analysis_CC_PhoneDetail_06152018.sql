...
CREATE PROCEDURE [dbo].[p_Link_Analysis_CC_PhoneDetail_06152018]
@FacilityID	int,
...
('P' + 
	...
	Update @result set TransRecordId = M.RecordID
		...
		Update @result set VFname = isnull(Left(FirstName, 25),''), VLName =  isnull(Left(LastName,25),'') 
		From @result  r, tblPrepaid p WITH(NOLOCK)
		where   p.PhoneNo = r.toShapeName 
		and SetId = 4 
...
Declare @XXXI Table (InmateID varchar(12) Null, toNo varchar(16) Null,  CallCount int)
Declare @YYYI Table (InmateID varchar(12) Null, toNo varchar(16) Null,  CallCount int)
Declare @PhoneCountI Table (toNo varchar(16) Null,  pCount int, totalCall int)
Declare @InmateCountI Table (InmateId varchar(16) Null, iCount int, totalCall int)
...
insert @XXXI(toNo, InmateID, CallCount)
				select tono, inmateID, sum(1) as callcount
				from @temp
				where toNo in (select ShapeName from @result where setId = 2 and ShapeContent like 'P%')
				group by toNo, InmateID

				--select toNo, InmateID, CallCount from  @XXXI

insert @PhoneCountI(toNo, pCount, totalCall)
select tono, sum(1) as pCount, sum(Callcount) as totalCall from @XXXI group by toNo;

insert @InmateCountI(inmateId, iCount, totalCall)
select InmateId, sum(1) as iCount, sum(Callcount) as totalCall from @XXXI group by InmateId;
...
Select distinct 1, toNo, InmateID, '', '', '' fromNo, '' Fname, '' LName, '' RecordId, '' ConnectDateTime, '' RecordFile, 0 duration, CallCount, LinkCount,'', '' as userLanguage
from @Temp3 where toNo in (select toNo from @XXXI)

insert @result ([SetId], [FromShapeName], [ToShapeName], [ShapeName], [ShapeContent], fromNo, Fname, Lname, RecordId, ConnectDateTime, RecordFile, duration,CallCount, linkcount,TransRecordId, userlanguage)
Select distinct 2, '', '', a.InmateID, 
			('I' + 
			cast(case when iCount > 2 then 3
			when iCount = 1 then 1 
			when iCount = 2 then 2
			else iCount end as varchar(1))),
		  '', Fname, LName, '' RecordId, '' ConnectDateTime, '' RecordFile, 0 duration, totalCall, iCount,'', '' as userlanguage
		
	from @Temp3 a, @InmateCountI b where a.InmateID =b.InmateID
			--order by InmateId
...
Select  distinct 2, '', '', a.toNo,
('P' + 
		cast(case when pCount > 2 then 3
			when pCount = 1 then 1 
			when pCount = 2 then 2
			else pCount end as varchar(1))),
		'' fromNo, '' Fname, '' LName, '' RecordId, '' ConnectDateTime, '' RecordFile, 0 duration, 
		 totalCall, pCount,'', '' as userlanguage
			from @temp3 b, @PhoneCountI a
			where a.toNo = b.toNo
...
--insert @result ([SetId], [FromShapeName], [ToShapeName], [ShapeName] , [ShapeContent], FromNo, Fname, Lname, RecordId, ConnectDateTime, RecordFile, 
--duration, CallCount, linkcount,TransRecordId, userLanguage)
...
--		 fromNo, Fname,  LName,  RecordId,  ConnectDateTime,  RecordFile, 
--		 CONVERT(varchar(8), DATEADD(ms, duration * 1000, 0), 114) as duration, CallCount, linkcount,
