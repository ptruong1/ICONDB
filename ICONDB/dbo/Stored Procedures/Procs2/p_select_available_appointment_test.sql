...
-- =============================================
CREATE PROCEDURE [dbo].[p_select_available_appointment_test]
	@facilityID	int,
	@locationID int,
	@InmateID	varchar(12),
...
BEGIN
	...
	SET @MaxFTime = 0;
	SET @InterApm =0;
	Select @MinTimeVisit = Isnull(MinDuration,0) ,@ApmIncrement =isnull(Increment,0), @MaxFTime = MaxDuration ,@hourBeforeVisit = HourBeforeVisit,@InterApm = isnull(InterSection,0)   from tblVisitFacilityConfig where FacilityID =@facilityID 
	...
	--if(@InterApm = 0 or @ApmIncrement =0)
	--	SET @MinTimeVisit = @MaxTimevisit;
