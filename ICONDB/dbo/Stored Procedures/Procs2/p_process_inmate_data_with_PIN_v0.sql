...
CREATE PROCEDURE [dbo].[p_process_inmate_data_with_PIN_v0]
@InmateID	varchar(12),
@PIN	varchar(12),
	 ...
	 Begin
		if (@status=1)
		 begin
			...
			UPDATE tblInmate SET  [status] = @status , PIN= @PIN,  modifyDate = getdate() ,LastName=@lastName ,FirstName =@firstName,MidName=@MidName, ReBook=1,RebookDate=GETDATE() where [Status] <4 and  [Status]  >1 and inmateID = @InmateID and FacilityID = @facilityID ;
			UPDATE tblInmate SET  [status] = @status , PIN= @PIN,  modifyDate = getdate() ,LastName=@lastName ,FirstName =@firstName,MidName=@MidName where [Status] =1 and inmateID = @InmateID and FacilityID = @facilityID ;
		 end
		else
