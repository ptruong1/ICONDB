...
-- =============================================
CREATE PROCEDURE [dbo].[p_select_TransferedInmatesByFacilityID]
(
	@FacilityID int
)
AS
...
FROM            tblInmate INNER JOIN tblInmateStatus ON tblInmate.Status = tblInmateStatus.statusID
	...
	WHERE tblInmate.facilityID = @FacilityID and statusID =3
ORDER BY LastName asc, tblInmate.inputdate DESC
