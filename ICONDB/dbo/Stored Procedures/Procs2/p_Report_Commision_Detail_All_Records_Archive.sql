CREATE PROCEDURE [dbo].[p_Report_Commision_Detail_All_Records_Archive]
@facilityID	int,
@AgentID	int,
	   ...
	   if @divisionID > 0
		if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
			select Calldate, Location, fromno,('#'+tono) as tono, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype , RecordDate, '1' as CallCount,  dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement )  as CallDuration,  CallRevenue, Pif,  (isnull(Rate,0)*100) BadDebtRate,  
				...
				from tblCallsBilledArchive with(nolock),  tblCalltype  with(nolock), tblBilltype  with(nolock)  , tblCommrateAgent with(nolock), tblBadDebt with(nolock) ,  tblFacility with(nolock), tblErrorType with(nolock)
				WHERE
								tblCallsBilledArchive.Billtype = tblBilltype.Billtype and
								tblCallsBilledArchive.errorCode = tblErrorType.ErrorType and 
			...
			select Calldate, Location, fromno,('#'+tono) as tono, tblbilltype.Descript as Billtype , tblCallType.Descript as Calltype , RecordDate, '1' as CallCount,  dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement )  as CallDuration,  CallRevenue, Pif,  (isnull(Rate,0)*100) BadDebtRate,  
				...
				(CASE when errorCode > 0 then 
				 ...
				 Else Null End)   CommAdjust
				from tblCallsBilledArchive with(nolock),  tblCalltype  with(nolock), tblBilltype  with(nolock)  , tblCommrate with(nolock), tblBadDebt with(nolock) ,  tblFacility with(nolock), tblErrorType with(nolock)
				WHERE
								tblCallsBilledArchive.Billtype = tblBilltype.Billtype and
								tblCallsBilledArchive.errorCode = tblErrorType.ErrorType and
								tblCallsBilledArchive.Calltype = tblCalltype.Abrev and
