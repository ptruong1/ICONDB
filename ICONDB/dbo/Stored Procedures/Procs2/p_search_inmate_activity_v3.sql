...
-- =============================================
CREATE PROCEDURE [dbo].[p_search_inmate_activity_v3]
@FacilityID int,
@InmateID	varchar(12),
 ...
 begin
	If(@InmateID <> '' and @FromDate <>'' and @todate<>'') 
	 begin
	    ...
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus ,RecordOpt) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0,(Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate) ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordDate>=@FromDate and RecordDate <=dateadd(d,1,@toDate); 	   
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), [status],RecordOpt from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID and Apmdate >=@FromDate and Apmdate <=dateadd(d,1,@toDate) ;
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
	 ...
	 begin
		insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType, Duration, CommStatus ,RecordOpt) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0,(Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordDate>=@FromDate ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID and RecordDate>=@FromDate ; 	   
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0),[status],RecordOpt from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and InmateID =@InmateID and Apmdate >=@FromDate  ;
	 ...
	 begin
		...
		insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType , Duration, CommStatus ,RecordOpt) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0,(Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID  ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock)  where inmateID = @InmateID and facilityID = @facilityID  ; 
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
	 ...
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus,RecordOpt ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0,(Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and RecordDate>=@FromDate and RecordDate <=dateadd(d,1,@toDate) ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and RecordDate>=@FromDate and RecordDate <= dateadd(d,1,@toDate); 	   
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), [status],RecordOpt from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and Apmdate >=@FromDate and Apmdate <=@toDate  ;
	 ...
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus,RecordOpt ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0, (Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0'and RecordDate>=@FromDate  ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and RecordDate>=@FromDate ; 	   
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
	 ...
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus,RecordOpt ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0,(Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and  RecordDate <=dateadd(d,1,@toDate) ; 	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0' and  RecordDate <=dateadd(d,1,@toDate); 	   
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID,isnull(visitDuration,0), [status],RecordOpt from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID and  Apmdate <=@toDate  ;
	 ...
	 begin
	    insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType, StationID , ContactPhone,PhoneType , Duration, CommStatus,RecordOpt ) 
			select RecordID, InmateID, RecordDate, 1, fromNo,tono,(CASE billType when '01' then 1 when '03' then 2 when '05' then 2 else  3 End), Duration,0,(Case RecordFile when 'NA' then 'N' Else 'Y' end)  from tblcallsbilled with(nolock)  where facilityID = @facilityID and InmateID <>'0'  ;	   
		 insert  @InmateAct  (RecordID , InmateID , ActivityDate, CommunityType,  StationID , ContactPhone,PhoneType , Duration, CommStatus ) 
			select isnull(RecordID,0), InmateID, RecordDate, 1, fromNo,tono,(CASE ResponseCode when '399' then 2 when '010' then 1 else 3 end), 0, errorType from tblcallsUnbilled with(nolock) where facilityID = @facilityID and InmateID <>'0';	   
		insert  @InmateAct  (RecordID , InmateID ,InmateName, ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus,RecordOpt ) 
			select ApmNo, InmateID,InmateName,  ApmDate+ apmtime , 2, StationID, EnduserID, isnull(visitDuration,0), [status],RecordOpt from tblVisitEnduserSchedule with(nolock)  where FacilityID =@facilityID  ;
		insert  @InmateAct  (RecordID , InmateID ,ActivityDate, CommunityType,  StationID , ContactPhone , Duration, CommStatus ) 
			select  (CASE MessageTypeID when 5 then [Message] Else cast(a.MailBoxID as varchar(10)) +'-' + CAST(a.MessageID as varchar(7)) end), b.InmateID,MessageDate,  (CASE MessageTypeID when 1 then 6 when 2 then 5 when 4 then 4 when 5 then 7 end), '',MessengerNo,60, (CASE MessageStatus when 1 then 1 when 2 then 1 else 0 end)
