...
CREATE PROCEDURE [dbo].[p_rerate_for_missing_rate_test]
...
select top 1  @RecordID =RecordID  ,  @facilityID= FacilityID, @fromNo = fromno, @tono = tono,@billtype = billtype, @callduration = duration   from tblcallsbilled   WHERE
	  ( billtype ='10'  Or  billtype ='03'  Or billtype ='05'  or billtype ='01'  )  and   datediff(hh,recorddate,getdate()) <300  and 
		CallRevenue =0
