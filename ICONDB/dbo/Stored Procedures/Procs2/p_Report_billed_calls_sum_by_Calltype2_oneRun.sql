CREATE PROCEDURE [dbo].[p_Report_billed_calls_sum_by_Calltype2_oneRun]
@FacilityID	int,
@AgentID	int,
...
Begin
	...
	Select   tblCalltype.Descript  as CallType
		,  Count(tblCallsBilledArchive_ComBined.Calltype) as CallsCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement)) as TotalMinutes,
		sum(CallRevenue ) As TotalRevenue
		  from tblCallsBilledArchive_ComBined  with(nolock), tblcalltype with(nolock), tblcommrate with(nolock)
		   where  tblCallsBilledArchive_ComBined.Calltype = tblCalltype.Abrev and
		   tblCallsBilledArchive_ComBined.FacilityID=  tblCommrate.FacilityID and
			tblCallsBilledArchive_ComBined.Billtype =  tblCommrate.billtype and
			tblCallsBilledArchive_ComBined.Calltype = tblCommrate.Calltype and
		      tblCallsBilledArchive_ComBined.FacilityID = @FacilityID   And  
		  tblCallsBilledArchive_ComBined.errorcode = '0' and  (RecordDate between @fromDate and dateadd(d,1,@todate) ) 
		  and convert (int,duration ) >5   
		  and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
		   Group by tblCalltype.Descript
end
