CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_By_Billtype2_Orig]
@facilityID	int,
@AgentID	int,
...
Else
 Begin 
  	If(@billtype	<> '')
		select tblbilltype.Descript as Billtype,  tblcallsbilled.calldate, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,2)))/60 as Numeric(12,2)) as CallDuration, sum(CallRevenue) CallRevenue, 0 as Pif,0 BadDebtRate,  
