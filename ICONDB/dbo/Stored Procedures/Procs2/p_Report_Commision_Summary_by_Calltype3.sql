CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_Calltype3]
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID ) > 0
 Begin
	If(@Calltype <>'')
		...
		group by  tblFacility.Location,tblCalltype.Descript,  tblCommrateAgent.CommRate,Rate,PifPaid 
		Order by tblFacility.Location,tblCalltype.Descript, tblCommrateAgent.CommRate,Rate,PifPaid 
	...
	If(@Calltype <>'')
		...
		from tblcallsbilled with(nolock),  tblcalltype with(nolock) , tblCommrateAgent with(nolock),tblFacility with(nolock)  where 
							...
							(RecordDate between @fromDate and dateadd(d,1,@todate) ) and tblcallsbilled.Calltype = @Calltype  and
							  CAST (ResponseCode as int) < 100  and convert (int,duration ) >15 
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
		group by  tblFacility.Location, tblCalltype.Descript,  tblCommrateAgent.CommRate,BadDebtRate,PifPaid 
		Order by tblFacility.Location,tblCalltype.Descript, tblCommrateAgent.CommRate,BadDebtRate,PifPaid 
