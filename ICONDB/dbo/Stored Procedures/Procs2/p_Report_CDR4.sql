CREATE PROCEDURE [dbo].[p_Report_CDR4]
@FacilityID	int,
@divisionID	int,
@locationID	int,
@phoneID	varchar(100),
...
begin
	If(@PIN <> '0' and  @callingCard <> ''  and  @DNI <> '' ) 
	 Begin
		...
		Select  left( tblANIs.StationID,20)  PhoneID, RecordID, toNo,   RecordDate as  ConnectDateTime , (case   tblcallsBilled.Billtype when '07' then left(CreditcardNo,12) else '' end ) DebitCardNo ,  tblcallsBilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType , 'Completed' as  State,
		...
		Select  left( tblANIs.StationID,20) PhoneID, 0 as RecordID , toNo,   RecordDate as  ConnectDateTime , DebitCardNo,  tblcallsUnbilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType  ,tblErrortype.Descript as  State,
		...
		select  RecordID, PhoneID , toNo , ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType  ,  State , Duration,CallRevenue, Descript    from  #tempCDR   WHERE   RecordIDs >= @startRowIndex
	...
	Else If(@FName <> '' and  @callingCard <> '') 
	 Begin
		Insert   #tempCDR (  PhoneID , RecordID,toNo , ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType  ,  State , Duration,CallRevenue, Descript  )
		 ...
		 tblcallsBilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType , 'Completed' as  State,CAST( CAST(duration as numeric(7,2))/60 as Numeric(9,2)) as Duration, CallRevenue, tblPhoneTypes.Descript as Descript
		...
		Select  left( tblANIs.StationID,20)  PhoneID, 0 as RecordID , toNo,   RecordDate as  ConnectDateTime , DebitCardNo,  tblcallsUnbilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType  ,tblErrortype.Descript as  State,
		 ...
		 select  left( tblANIs.StationID,20)  PhoneID, 0 as RecordID , 'CTS'  as Tono ,      ReportTime   as ConnectDateTime   ,''  DebitCardNo,  tblincidentreport.PIN,  FirstName, LastName ,   'CTS'  as BillType ,    'Call To Staff'      as     state     ,
			  '0' as Duration, 0  as  CallRevenue, '3' as Descript  from tblincidentreport with(nolock) , tblInmate with(nolock)  ,tblANIs with(nolock)  
			where 	ANI = tblANIs.ANINo and
			tblincidentreport.PIN = tblInmate.Pin and
			tblincidentreport.FacilityID	= @FacilityID and tblincidentreport.InputDate  >= @FromDate  
	...
	Else If(@LName <> '' and  @callingCard <> '') 
	 Begin
		...
		Select  left( tblANIs.StationID,20) PhoneID, 0 as RecordID , toNo,   RecordDate as  ConnectDateTime , DebitCardNo,  tblcallsUnbilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType  ,tblErrortype.Descript as  State,
		 ...
		 select  left( tblANIs.StationID,20) , 0 as recordID, 'CTS'  as Tono ,      ReportTime   as ConnectDateTime   ,'' DebitCardNo,  tblincidentreport.PIN,  FirstName, LastName ,   'CTS'  as BillType ,    'Call To Staff'      as     state     ,
			  '0' as Duration, 0  as  CallRevenue, '3' as Descript  from tblincidentreport with(nolock) , tblInmate with(nolock)  ,tblANIs with(nolock)  
			where 	ANI = tblANIs.ANINo and
			tblincidentreport.PIN = tblInmate.Pin and
			tblincidentreport.FacilityID	= @FacilityID and tblincidentreport.InputDate  >= @FromDate  
...
begin
	...
	Else If(@PIN <> '0') 
	 Begin
		...
		Select  left( tblANIs.StationID,20) , 0 as RecordID, toNo,   RecordDate as  ConnectDateTime , DebitCardNo,  tblcallsUnbilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType  ,tblErrortype.Descript as  State,
			 '0'  as Duration,0, tblPhoneTypes.Descript as Descript    from tblcallsUnbilled with(nolock)  ,  tblBilltype with(nolock) ,tblInmate with(nolock) ,tblANIs with(nolock) ,tblPhonetypes with(nolock) , tblErrortype  with(nolock)
		  where  tblcallsUnBilled.fromno = tblANIs.ANINo and
			 tblcallsUnBilled.PIN  = tblInmate.PIN and
			   tblcallsUnBilled.Billtype = tblBilltype.billtype and  isnull(tblcallsUnBilled.PhoneType,'3') = tblPhoneTypes.Phonetype and 
...
left(CreditcardNo,12) else '' end ) DebitCardNo,  tblcallsBilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType , 'Completed' as  State,
			CAST( CAST(duration as numeric(7,2))/60 as Numeric(9,2)) as Duration, CallRevenue, tblPhoneTypes.Descript as Descript    from tblcallsBilled with(nolock)  , 
