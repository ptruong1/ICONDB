CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_FromNo2]
...
Else
	...
	If(@fromNo <> '')
		...
		select  FromNo, tblcallsbilled.calldate, count(CallRevenue ) CallCount, CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(12,4)) as CallDuration, sum(CallRevenue) CallRevenue, Sum(Pif) as Pif, '0' BadDebtRate, 0 as Rate,  
		...
		from tblcallsbilled with(nolock), tblCommRateAgent with(nolock)  where 
							...
							tblcallsbilled.agentID = @agentID and
							 ...
							 CAST (ResponseCode as int) < 100  and convert (int,duration ) >15 
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
		group by tblcallsbilled.calldate,FromNo,tblCommrateAgent.CommRate,BadDebtRate, PifPaid 
		Order by tblcallsbilled.calldate,FromNo,tblCommrateAgent.CommRate,BadDebtRate ,PifPaid
