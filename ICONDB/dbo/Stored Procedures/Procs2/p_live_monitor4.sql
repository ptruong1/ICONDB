...
CREATE PROCEDURE p_live_monitor4
@facilityID	int,
@DivisionID       int,
@locationID	int,
...
If(@stationID <> '')
	...
	From tblANIs A With(nolock) left outer join tblOncalls O with(nolock) on A.ANIno =  O.fromNo left outer join tblACPs P  with(nolock) on A.facilityID =O.FacilityID
	WHERE 
