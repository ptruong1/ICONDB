...
-- =============================================
CREATE PROCEDURE [dbo].[p_UPDATE_Inmate_Infomations_11032017]
(
	@PIN Varchar(12),
	@InmateID Varchar(12),
...
SET NOCOUNT OFF;
	...
	Select      @preLastName=isnull([LastName],''), 
				...
				@PrePrimaryLanguage =ISNULL([PrimaryLanguage], 0)
	...
	if (@InmateNote <>'')
		begin
			...
			INSERT INTO tblInmateNote ([NoteID],[NoteTypeID],[FacilityID] ,[InmateID], [Note], [InputDate], [UserName])
			 VALUES (@ID ,1,@FacilityId, @InmateID, @InmateNote, getdate(), @UserName);
		end
	...
	----Check all Fields--------
       ...
       if(@PrePrimaryLanguage <>@PrimaryLanguage and @PrimaryLanguage is not null)
			SET @WhatEdit =  @WhatEdit  + '; Primary Language Changed: ' +  cast(@prePrimaryLanguage as  varchar(2))  + '-->' + cast(@PrimaryLanguage as  varchar(2)) ;
	-----------
		if(@WhatEdit <>'')
		 begin
