...
CREATE PROCEDURE  [dbo].[p_user_Icon5_login1]
@userName  varchar(20),
@password	varchar(20),
...
Begin
	...
	SET @loginID = @@identity;
	 ...
	 -- End
	select F.FacilityID,  F.Location ,  F.Address, F.City, F.State, F.Phone , F.logo, F.PINRequired, F.AgentID,  
		@loginID as LoginID , @userLevel as  userLevel , @transcriptOpt as  transcriptOpt, @authID as authID  , isnull( o.VisitOpt,0)  VisitOpt, @FirstName as FirstName, @LastName as LastName
	from tblFacility F with(nolock) left outer join leg_Icon.dbo.tblvisitFacilityConfig O with(nolock)
	On 
	F.FacilityID = o.FacilityID 
	where 
...
Else IF   (@userLevel = 2  )
 Begin
	...
	--Insert tblUserLogs(UserName,Password,IPAddess,loginTime) 	Values(@userName,@password,@IPaddress,getdate());
	SET @loginID = @@identity ;
	select 0 as FacilityID, Company as  Location ,  Address1, City, StateCode as State, Phone , ''  as Logo ,0  as PINRequired, AgentID,  @loginID as LoginID ,@userLevel as  userLevel ,@transcriptOpt as  transcriptOpt,  @authID as authID ,@visitOpt as VisitOpt, @FirstName as FirstName, @LastName as LastName    from tblAgent with(nolock)
	where AgentID = @agentID;
  End
Else 
 Begin
	...
	@transcriptOpt as  transcriptOpt,  @authID as authID ,@visitOpt as VisitOpt, @FirstName as FirstName, @LastName as LastName ;
  End
 EXEC [p_get_facility_time] @facilityID ,@ActTime OUTPUT ;
