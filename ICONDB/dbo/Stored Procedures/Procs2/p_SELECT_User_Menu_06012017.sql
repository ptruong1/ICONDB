...
CREATE PROCEDURE [dbo].[p_SELECT_User_Menu_06012017]
( 
	@AuthID int
)
AS
SELECT  
	...
	,isnull( tblAuthUsers.FacilityConfig,0) "Facility"
      ...
      ,isnull(tblAuthFormTab.[InmateKite], 1) "Inmate Kite"
	  ,isnull(tblAuthFormTab.[MedicalKite], 1) "Medical Kite"
	  ,isnull(tblAuthFormTab.[Grievance], 1) "Grievance"
	  ,isnull(tblAuthFormTab.[LegalForm], 1) "Legal Form"
