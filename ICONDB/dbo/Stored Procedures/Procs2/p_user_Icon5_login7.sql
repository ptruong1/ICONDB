...
CREATE PROCEDURE  [dbo].[p_user_Icon5_login7]
@userName  varchar(20),
@password	varchar(20),
...
Declare @loginID	bigint, @FacilityID int, @u varchar(20) , @p varchar(20), @agentID int , @userLevel   tinyint , @transcriptOpt	tinyint, @ActTime datetime, @FormsOpt tinyint,
	...
	Declare @return_value int, @nextID int, @ID int, @tblUserLogs nvarchar(32), @UserPhone varchar(10), @Email nvarchar(50), @SecondFactor bit, @userGroupID int, @MasterUserGroupID int, @iconURL varchar(25)  ;
SET @FacilityID = 0;
SET @agentID =0 ;
SET @userLevel  =0;
SET @transcriptOpt	= 0;
...
Begin
	...
	select F.FacilityID,  F.Location ,  F.Address, F.City, F.State, F.Phone , F.logo, F.PINRequired, F.AgentID, isnull( F.groupID,0)  GroupID,
			@loginID as LoginID , @userLevel as  userLevel , @transcriptOpt as  transcriptOpt, @authID as authID  ,
			 ...
			 @SecondFactor as SecondFactor, @userGroupID as UserGroupID, @MasterUserGroupID as MasterUserGroupID,'http://207.141.247.184:7777/default.aspx?' as iconURL 
 ...
 Begin
	...
	if (@agentID =1) 
		...
		select 1 as FacilityID, Company as  Location ,  Address1, City, StateCode as State, 
				...
				@LastName as LastName,@FormsOpt as FormsOpt, @UserPhone as UserPhone, @Email as Email, @SecondFactor as SecondFactor, @userGroupID as UserGroupID, @MasterUserGroupID as MasterUserGroupID,'http://207.141.247.187:8888/default.aspx?' as iconURL 
		...
		select 0 as FacilityID, Company as  Location ,  Address1, City, StateCode as State, 
				Phone , ''  as Logo ,0  as PINRequired, AgentID, 0 as GroupID,  @loginID as LoginID ,@userLevel as  userLevel ,
				@transcriptOpt as  transcriptOpt,  @authID as authID ,@visitOpt as VisitOpt, @FirstName as FirstName, 
				@LastName as LastName,@FormsOpt as FormsOpt, @UserPhone as UserPhone, @Email as Email, @SecondFactor as SecondFactor, @userGroupID as UserGroupID, @MasterUserGroupID as MasterUserGroupID,'http://207.141.247.184:7777/default.aspx?' as iconURL 
		from tblAgent with(nolock)
		where AgentID = @agentID;
		end
...
Else 
 Begin
	...
	@transcriptOpt as  transcriptOpt,  @authID as authID ,@visitOpt as VisitOpt, @FirstName as FirstName, @LastName as LastName,@FormsOpt as FormsOpt, @UserPhone as UserPhone, @Email as Email, @SecondFactor as SecondFactor, @userGroupID as UserGroupID, @MasterUserGroupID as MasterUserGroupID, 'http://207.141.247.184:7777/default.aspx' as iconURL ;
  End
