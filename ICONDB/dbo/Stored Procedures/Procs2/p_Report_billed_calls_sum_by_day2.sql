CREATE PROCEDURE [dbo].[p_Report_billed_calls_sum_by_day2]
@FacilityID	int,
@AgentID	int,
...
Begin
		...
		Select   Location, Calldate ,  Count(calldate) as CallsCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement)) as TotalMinutes, 
		...
		(RecordDate between @fromDate and dateadd(d,1,@todate) ) and convert (int,duration ) >5
  		and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0)
		   Group by Location, Calldate
		   Order by Location, Calldate
		  else
		  Select   Location, Calldate ,  Count(calldate) as CallsCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement)) as TotalMinutes, 
 			sum(CallRevenue ) As TotalRevenue
		 from tblcallsBilled  with(nolock), tblFacility with(nolock)
...
Begin
	if(select count(*) from tblCommRateAgent with(nolock) where AgentID= @AgentID) > 0
	Select   Calldate ,  Count(calldate) as CallsCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement)) as TotalMinutes,
 			sum(CallRevenue ) As TotalRevenue
