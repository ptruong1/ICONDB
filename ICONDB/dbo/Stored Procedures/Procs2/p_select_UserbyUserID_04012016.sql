...
CREATE PROCEDURE [dbo].[p_select_UserbyUserID_04012016]
(
	@FacilityID int,
	@UserID varchar(20)
)
...
IF @FacilityID = 1
	BEGIN
		SELECT U.facilityID, A.AuthID, UserID, @pwd as Password , LastName, FirstName, MidName,ID, AgentID, Department, Phone, Email, U.inputdate,
...
ELSE
	BEGIN
		SELECT  U.facilityID, A.AuthID, UserID, @pwd as password, LastName, FirstName, MidName,ID, Department, Phone, Email,U.UserLevel, U.inputdate, 
			   ISNULL(Admin, 0) as Admin,
			   ISNULL(PowerUser,0) As PowerUser,
			   ISNULL([Finance-Auditor],0) as [Finance-Auditor],
			   ISNULL(Investigator,0) as Investigator,
