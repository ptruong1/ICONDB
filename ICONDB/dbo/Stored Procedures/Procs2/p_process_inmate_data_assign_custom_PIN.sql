...
CREATE PROCEDURE [dbo].[p_process_inmate_data_assign_custom_PIN]
@InmateID	varchar(12),
	...
	if(@BookingID <>'' and @BookingID is not null )
	 begin
		if(select count(*) from tblInmateInfo with(nolock)  where BookingNo= @BookingID and PIN = @InmateID and FacilityID = @facilityId) =0		
		 begin
			insert  tblInmateInfo (BookingNo, PIN, FacilityID, BirthDate,Location)
					values(@BookingID , @InmateID , @facilityId, @DOB, @AtLocation);
		 end
		 --if(select count(*) from  tblInmateBookInfo with(nolock) where BookingNo= @BookingID and PIN = @InmateID and FacilityID = @facilityId and BAILAMOUNT =@BailAmount) =0
			-- insert  tblInmateBookInfo (BookingNO,PIN , chargecode, FacilityID , BAILAMOUNT , ApperanceDate)
			--			values(@BookingID, @PIN,@BailAmount, @facilityId,@BailAmount,@CounrtDate) ;
