CREATE PROCEDURE [dbo].[p_Report_Houston_ElectricMessage_By_InmateID_New] 
(
	@facilityID	int,
...
left join tblMailBox  t1  with(nolock) on t1.MailBoxID = t2.MailBoxID and t1.FacilityID = @facilityid	
left join tblInmate  t3  with(nolock) on t1.InmateID= t3.InmateID and t1.FacilityID = @facilityid 	
