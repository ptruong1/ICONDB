...
-- =============================================
CREATE PROCEDURE [dbo].[p_select_available_appointment_old]
	@facilityID	int,
	@locationID int,
	@InmateID	varchar(12),
...
BEGIN
	...
	Declare @MaxTimevisit smallint, @LocLimitTime smallint, @InmateLimitTime smallint,@DayLimitTime smallint, @RelationShipID tinyint;
	Declare	@FromTime1 varchar(5), @toTime1 varchar(5),@FromTime2 varchar(5), @toTime2 varchar(5), @FromTime3 varchar(5), @toTime3 varchar(5);
	...
	SET @LocLimitTime =0;
	SET @DayLimitTime=0;
	Select @MinTimeVisit = Isnull(MinDuration,0) ,@ApmIncrement =isnull(Increment,1), @MaxTimevisit = MaxDuration ,@hourBeforeVisit = HourBeforeVisit  from tblVisitFacilityConfig where FacilityID =@facilityID 
	...
	--if(@locationID =0) 
		Select @locationID = locationID,@InmateLimitTime  =ISNULL(MaxVisitTime,0), @VisitPerDay = isnull(VisitPerDay,0), @VisitPerWeek=isnull(VisitPerWeek,0) ,@VisitPerMonth=isnull(VisitPerMonth,0) FROM tblVisitInmateConfig where InmateID =@InmateID  and FacilityID =@facilityID ;
	...
	if(select COUNT(*) from tblVisitLocationSchedule where FacilityID =@facilityID and LocationID =@locationID) >0
	 begin
	    --select 'TEST'
		SELECT @period = isnull(LimitTime,15), @InterApm= InterApm  , @FromTime1= fromTime1, @toTime1 = totime1, @FromTime2= FromTime2,@toTime2=toTime2,
		 @FromTime3= fromTime3, @toTime3 = totime3, @LocLimitTime= LimitTime  from [leg_Icon].[dbo].[tblVisitLocationSchedule]
		    where FacilityID = @facilityID and LocationID =@locationID and scheduleday= @dw and visitType =2;
	 end 
	else
		SELECT @period = isnull(LimitTime,15), @InterApm= InterApm  , @FromTime1= fromTime1, @toTime1 = totime1, @FromTime2= FromTime2,@toTime2=toTime2,@DayLimitTime= LimitTime,  @FromTime3= fromTime3, @toTime3 = totime3 from [leg_Icon].[dbo].[tblVisitFacilitySchedule]
