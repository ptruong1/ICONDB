...
CREATE PROCEDURE [dbo].[p_update_UserbyID11272018]
(	
@FacilityID int ,
@Password  varchar(20),
@UserID varchar(20),
	...
	BEGIN
		...
		--exec [dbo].[LegEncrypt] @UserID,	@EnCryptUser  OUTPUT
				...
				UPDATE [tblUserprofiles] SET [UserID] = @UserID, [LastName] = @LastName, [FirstName] = @FirstName, [MidName] = @MidName,[ID] = @ID, [Department] = @Department, [Phone] = @Phone, [Email] = @Email ,
												  [Password] = @password , [status] = @status, [IPaddress]=@IPaddress, UserIDDEC= @EnCryptUser, PasswordDEC = @EnCryptPass, SecondFactor = @SecondFactor, UserGroupID=@UserGroupID, MasterUserGroupID=@MasterUserGroupID
				WHERE [UserID] = @Original_UserID; 
		  end
		else
			UPDATE [tblUserprofiles] SET [UserID] = @UserID, [LastName] = @LastName, [FirstName] = @FirstName, [MidName] = @MidName,[ID] = @ID, [Department] = @Department, [Phone] = @Phone, [Email] = @Email ,
												   [status] = @status, [IPaddress]=@IPaddress, SecondFactor = @SecondFactor, UserGroupID=@UserGroupID, MasterUserGroupID= @MasterUserGroupID
													WHERE [UserID] = @Original_UserID; 
