CREATE PROCEDURE [dbo].[p_Report_Commision_Summary_by_Division1]
...
If (@AgentId >1 AND @facilityID =0 ) and (select count(*) from tblBadDebt with(nolock) where  AgentID = @AgentID )= 0
 Begin
	If(@divisionID >0)
		...
		select  tblFacilityDivision.DepartmentName as DiVision, tblcallsbilled.calldate, count(CallRevenue ) CallCount,  CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(12,4)) as CallDuration, sum(CAST(CallRevenue as numeric(9,4))) CallRevenue, Sum(Pif) as Pif, '0' BadDebtRate, 0 as Rate,
		...
		from tblcallsbilled with(nolock), tblcommrateAgent with(nolock),  tblFacilityDivision  with(nolock) , tblANIs  with(nolock) where 							
							tblcallsbilled.AgentID = tblCommrateAgent.AgentID AND
							 tblcallsBilled.errorcode = '0' and 
							tblcallsbilled.Billtype =  tblCommrateAgent.billtype and
							 tblcallsbilled.Calltype = tblCommrateAgent.Calltype and 
							...
							tblcallsbilled.agentID = @agentID and
							 ...
							 CAST (ResponseCode as int) < 100  and convert (int,duration ) >5 
							and  tono not in (select AuthNo from tblOfficeANI where  Billabe =0) 
		group by  tblFacilityDivision.DepartmentName, tblcallsbilled.calldate,tblcommrateAgent.CommRate,BadDebtRate, PifPaid 
		Order by  tblFacilityDivision.DepartmentName, tblcallsbilled.calldate,tblcommrateAgent.CommRate,BadDebtRate, PifPaid 
...
Else
 Begin
	If(@divisionID >0)
		...
		select  tblFacilityDivision.DepartmentName as DiVision, tblcallsbilled.calldate, count(CallRevenue ) CallCount,  CAST(sum( CAST(duration as numeric(12,4)))/60 as Numeric(12,4)) as CallDuration, sum(CallRevenue) CallRevenue, 0 as Pif,0 BadDebtRate,  
