CREATE PROCEDURE [dbo].[p_Report_billed_calls_sum_by_ANI2]
@FacilityID	int,
@AgentID	int,
...
Begin
	...
	Select    Location, FromNo
		...
		,  Count( FromNo) as CallsCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement)) as TotalMinutes,
		sum(CallRevenue ) As TotalRevenue
		  from tblcallsBilled  with(nolock), tblANIs with(nolock), tblFacility with(nolock), tblCommrate with(nolock)
		   where fromNo = ANIno and  tblcallsBilled.AgentID = @AgentID  And  tblcallsBilled.errorcode = '0' and 
			tblCallsBilled.facilityID = tblfacility.FacilityID and
			tblcallsbilled.AgentID = tblCommrate.AgentID AND
				 tblcallsbilled.FacilityID=  tblCommrate.FacilityID and
...
Select    FromNo
		,  Count( FromNo) as CallsCount, sum(dbo.fn_calculateBillableTime (duration, minDuration ,minIncrement)) as TotalMinutes,
		sum(CallRevenue ) As TotalRevenue
		  from tblcallsBilled  with(nolock), tblANIs with(nolock), tblCommrateAgent with(nolock)
