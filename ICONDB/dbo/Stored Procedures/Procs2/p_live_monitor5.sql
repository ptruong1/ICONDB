CREATE PROCEDURE [dbo].[p_live_monitor5]
@facilityID	int,
@DivisionID       int,
@locationID	int,
...
If(@stationID <> '')
	...
	select O.facilityID, isnull(RecordID,0) RecordID, (CASE Isnull(ToNo,'') When '' then 'Inactive' Else 'Active' End) Status,  	Isnull(PIN,0) as PIN, StationID, isnull(Tono,'') Tono , isnull(CAST(Recorddate AS Varchar(30)),'') ConnectDateTime ,
	...
	From tblANIs A With(nolock) left outer join tblOncalls O with(nolock) on (A.ANIno =  O.fromNo and  A.facilityID =O.FacilityID) , tblACPs P  with(nolock)
	WHERE 
