...
CREATE PROCEDURE [dbo].[p_payment_prepaid_Account]
...
AS
SET NoCount ON
declare  @purchaseID bigint, @authAmount	numeric(7,2),   @LastBalance  numeric(7,2) , @EnduserID int
Declare @return_value int, @nextID bigint, @ID bigint, @tblPrepaidPayments nvarchar(32), @tblTransactionLogs nvarchar(32), @tblpurchase nvarchar(32),@tblWUPrepaid nvarchar(32);
SET  @authAmount = @callAmount + @ProcessFee + @tax + @setupFee
SET  @LastBalance  = 0
SET @EndUserID =0
 if (@paymentTypeID = 4)
 Begin
   	if(select count(*) from  tblWUPrepaid  with(nolock) where CustAcctNo =@AccountNo  ) = 0
		...
		values(@ID, @AccountNo ,left(@BilltoFirstName,16)	,@BilltoLastName  ,left(@BillToAddress,40) ,@BillToCity , @billToState, @BillToZip,@BillToCountry,@AccountNo ,'A',  @facilityID,	  0, 0) 
...
Else
 Begin
	...
	If(@decision = 'ACCEPT'  or   @decision = 'Accept'   or   @decision = 'accept'  or  @decision = 'Approved' or  @decision ='APPROVED')   
	 BEGIN
       ...
       set           @ID = @nextID ;    
		INSERT INTO tblPrepaidPayments(paymentID, AccountNo, FacilityID,  Amount   ,    PaymentTypeID, CheckNo  ,  CCNo  , CCExp ,CCzip ,CCcode, CCFirstName,CCLastName ,  UserName ,  PaymentDate, PurchaseNo,LastBalance  )
					        Values(@ID, @AccountNo, @FacilityID, @authAmount , @PaymentTypeID, @checkno, @ccNo, @ccExp,@BilltoZip, @ccCVV, @BillToFirstName,@BillToLastName	,@userName, getdate(), @purchaseID, @LastBalance)
		If(@EnduserID = 0)
		 begin
...
--		 begin
--			insert tblpurchasedetail( PurchaseNo,  DetailCat, Detailtype, Amount  )  values( @purchaseID, 1, 4, @Tax);
--			EXEC [p_CalculatePrepaidTax] @CallAmount ,@AccountNo,@Tax  ,@purchaseID;
--		 end
