CREATE PROCEDURE [dbo].[p_Report_CDR_BillType_Selected9_test]
...
Duration,CallRevenue, Descript, RecordFile  )
		Select  left( tblANIs.StationID,20) PhoneID, RecordID , toNo,   DATEADD(second, - duration, recordDate) as startTime, RecordDate as  
ConnectDateTime ,  (case   tblcallsBilled.Billtype when '07' then left(CreditcardNo,12) else '' end )  DebitCardNo,  tblcallsBilled.PIN, 
...
ConnectDateTime, DebitCardNo,  tblcallsUnbilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType,
			tblCallType.Descript  as CallType,tblErrortype.Descript as  State,'0'  as Duration,0, tblPhoneTypes.Descript as Descript , 'UnK' as 
RecordFile   
		from tblcallsUnbilled with(nolock), tblBilltype with(nolock), tblInmate with(nolock) ,tblANIs with(nolock) ,tblPhonetypes with(nolock) , 
tblErrortype  with(nolock) , tblCallType with(nolock)
...
ConnectDateTime   ,''  DebitCardNo, tblincidentreport.PIN, LastName ,  FirstName,   'CTS'  as BillType,
			 'RTS' as CallType ,    'Call To Staff'      as     state ,  '0' as Duration, 0 as  CallRevenue, '3' as Descript, 'UnK' as RecordFile 
			 FROM  tblincidentreport with(nolock) , tblInmate with(nolock)  	,tblANIs with(nolock)  
			 WHERE ANI = tblANIs.ANINo and tblincidentreport.PIN = tblInmate.Pin and
				tblincidentreport.FacilityID = @FacilityID and tblincidentreport.InputDate  >= @FromDate  
		...
		select  RecordID, PhoneID , toNo , StartTime, ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType  ,  State , 	Duration,CallRevenue, 
Descript, RecordFile    from  #tempCDR   WHERE   RecordIDs >= @startRowIndex
	...
	Else If @FName <> ''  ------- 04/25/2011 ----
              Begin
		Insert   #tempCDR (  PhoneID , RecordID,toNo , StartTime, ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType, CallType  ,  
...
ConnectDateTime   ,''  DebitCardNo, tblincidentreport.PIN, LastName ,  FirstName,   'CTS'  as BillType,
		...
		select   RecordID, PhoneID , toNo , StartTime, ConnectDateTime , CallingCard, PIN,  Fname, LName ,  BillType, CallType  ,  State , 
...
tblincidentreport.PIN, LastName ,  FirstName,   'CTS'  as BillType,
			 'RTS' as CallType ,    'Call To Staff'      as     state ,  '0' as Duration, 0 as  CallRevenue, '3' as Descript, 'UnK' as RecordFile 
			 FROM  tblincidentreport with(nolock) , tblInmate with(nolock)  	,tblANIs with(nolock)  
			 WHERE ANI = tblANIs.ANINo and tblincidentreport.PIN = tblInmate.Pin and
				tblincidentreport.FacilityID = @FacilityID and tblincidentreport.InputDate  >= @FromDate  
...
ConnectDateTime ,(case   tblcallsBilled.Billtype when '07' then left(CreditcardNo,12) else '' end )  DebitCardNo,  
			tblcallsBilled.PIN,  FirstName, LastName ,   tblBilltype.Descript  as  BillType, tblCallType.Descript  as  CallType , 'Completed' as  
State,
			CAST( CAST(duration as numeric(7,2))/60 as Numeric(9,2)) as Duration, CallRevenue, tblPhoneTypes.Descript as Descript , RecordFile
   			from tblcallsBilled with(nolock)  ,  tblBilltype with(nolock), tblCallType with(nolock) ,tblInmate with(nolock) ,tblANIs with(nolock) 
,tblPhonetypes with(nolock) 
...
Descript, RecordFile    from  #tempCDR   WHERE   RecordIDs >= @startRowIndex
 ...
 End
else If(@LocationID > 0)    ----------------------------- Location  -Phone ID = ''
begin
	If(@PIN <> '0' and  @callingCard <> ''  and  @DNI <> '' ) 
	 Begin
		Insert   #tempCDR (  PhoneID ,RecordID, toNo , StartTime, ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType, CallType  ,  	State 
...
ConnectDateTime      ,''  DebitCardNo, tblincidentreport.PIN, LastName ,  FirstName,   'CTS'  as BillType,
		...
		select  RecordID, PhoneID , toNo , StartTime, ConnectDateTime , CallingCard,PIN,  Fname, LName ,  BillType  ,  State , Duration,CallRevenue, Descript, 
...
Duration,CallRevenue, Descript, RecordFile  )
		...
		Select  left( tblANIs.StationID,20)  PhoneID, RecordID , toNo,   DATEADD(second, - duration, recordDate) as startTime, RecordDate as  
ConnectDateTime ,(case   tblcallsBilled.Billtype when '07' then 	left(CreditcardNo,12) else '' end ) DebitCardNo,  tblcallsBilled.PIN,  FirstName, 
...
when '07' then left(CreditcardNo,12) else '' end )  DebitCardNo,  tblcallsBilled.PIN,  FirstName, LastName ,   
		tblBilltype.Descript  as  BillType, tblCallType.Descript  as  CallType , 'Completed' as  State,CAST( CAST(duration as numeric(7,2))/60 as 
Numeric(9,2)) as Duration, CallRevenue, tblPhoneTypes.Descript as Descript , RecordFile
   		from tblcallsBilled with(nolock)  ,  tblBilltype with(nolock), tblCallType with(nolock) ,tblInmate with(nolock) ,tblANIs with(nolock) 
,tblPhonetypes with(nolock) 
...
tblErrortype  with(nolock) , tblCallType with(nolock)
		 ...
		 select  left( tblANIs.StationID,20)  PhoneID, 0 as RecordID , 'CTS'  as Tono ,      ReportTime as StartTime,  ReportTime   as 
ConnectDateTime      ,''  DebitCardNo ,  tblincidentreport.PIN,  FirstName, LastName ,   
...
'UnK' as RecordFile  from tblincidentreport with(nolock) , tblInmate with(nolock)  ,tblANIs with(nolock)  
			...
			tblincidentreport.FacilityID = @FacilityID and tblincidentreport.InputDate  >= @FromDate  
			and  convert(varchar(10), tblincidentreport.InputDate  ,101)  <= @toDate   and @private =1 
			Order by  ConnectDateTime Desc 
