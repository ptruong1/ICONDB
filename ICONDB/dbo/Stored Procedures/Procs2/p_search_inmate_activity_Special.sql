...
-- =============================================
CREATE PROCEDURE [dbo].[p_search_inmate_activity_Special]
@FacilityID int,
@InmateID	varchar(12),
...
if(select count(*) from @InmateAct ) >0
 ...
 Select RecordID , InmateID ,isnull(InmateName,'') as InmateName, ActivityDate, b.Description as ActivityType, isnull( StationID,'')  StationID, ContactPhone ,
  isnull( ContactName, '')  ContactName ,  (CASE a.CommunityType when 5 then 'NA' else  [dbo].[fn_ConvertSecToMin] ( Duration) end)  Duration, 
			ActivityStatus,c.Descript Phonetype, RecordOpt, Row_Number() Over(Order By [InmateID] Desc) As RowNum

			Into tblTempUser

	from  @InmateAct a, tblCommunicationType b, tblPhoneTypes c
	where a.CommunityType = b.commID  and a.PhoneType = c.PhoneType
	group by  InmateID ,InmateName, ActivityDate,  StationID, ContactPhone ,  ContactName ,  Duration, CommStatus ,
	RecordID, b.Description,ActivityStatus,  a.CommunityType,c.Descript, RecordOpt
	Order by  ActivityDate, InmateID 	 Desc ;

	set  @RecCount = (select count(*) from  tblTempUser) 
	  
	select @sql = 'if OBJECT_ID(''' + @table + ''') is not null drop table ' + @table  
	exec sp_executesql @sql

	set @Sql = 'sp_rename ' + '''tblTempUser''' + ',' + '''' + @table + ''''
...
if @StrFilter = ''
	...
	Duration varchar(20), CommStatus Varchar(30), RecordOpt varchar(1) Default 'Y', ActivityStatus varchar(30), rowNum int) ;

set @sql = 'insert  #Temp2  (RecordID , InmateID ,InmateName, ActivityDate, ActivityType, StationID , ContactPhone , Phonetype, ContactName, Duration, CommStatus ,
		RecordOpt, rowNum ) 
	...
	select @RecCount as CountNoFilter
	...
	set @sql = 'insert  #Temp3  (RecordID , InmateID ,InmateName, ActivityDate, ActivityType, StationID , ContactPhone , Phonetype, ContactName, Duration, CommStatus ,
		RecordOpt, rowNum ) 
		Select RecordID , InmateID ,InmateName, ActivityDate,ActivityType, StationID, ContactPhone , Phonetype,
	   ContactName , Duration, 	ActivityStatus, RecordOpt, Row_Number() Over(Order By [ActivityDate],[InmateID] Desc) As RowNum
	...
	--select @RecCount as countwithFilter
	
	set @sql = 'Select RecordID , InmateID ,InmateName, ActivityDate, ActivityType, StationID, ContactPhone , Phonetype,
	   ContactName , Duration, 	ActivityStatus, RecordOpt,  RowNum
		 from #Temp3 where (rowNum  >=  @startRow and rowNum <  @MaxRow) and ' + @StrFilter + ' order by rowNum'

		print @Sql
		exec sp_executesql @sql, N'@startRow int, @MaxRow int,@StrFilter nvarchar(200)',  @startRow =  @startRow,  @MaxRow =  @MaxRow, @StrFilter=@StrFilter
