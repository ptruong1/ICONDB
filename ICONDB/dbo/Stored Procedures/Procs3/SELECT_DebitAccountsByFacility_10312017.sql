...
CREATE PROCEDURE [dbo].[SELECT_DebitAccountsByFacility_10312017]
(
	@FacilityID int
)
AS
...
FROM tblDebit   with(nolock)  INNER JOIN tblStatus  with(nolock) ON tblDebit.status = tblStatus.statusID
			left JOIN tblInmate with (nolock) ON (tblDebit.InmateID = tblInmate.InmateID and  tblDebit.FacilityID = tblInmate.FacilityID )
