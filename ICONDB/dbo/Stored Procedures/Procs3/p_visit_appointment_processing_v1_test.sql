...
-- =============================================
CREATE PROCEDURE [dbo].[p_visit_appointment_processing_v1_test]
@ApmNo int,
...
AS
BEGIN
Declare @currentApm smallint, @scheduleDate smalldatetime,@ScheduleTime time(0), @NumOfStations smallint,  @ApprovedApm bit, @visitorID int, @RecordOpt varchar(1),
	@locationID int, @iCallSp int, @ScheduleStatus tinyint, @FacilityContactPhone varchar(10), @FacilityContactEmail varchar(50), @ApprovedBy varchar(10) ,@visitType tinyint;
...
select @scheduleDate =ApmDate,@ScheduleTime=ApmTime,@locationID=locationID, @visitorID =VisitorID, @visitType = visitType from [leg_Icon].[dbo].[tblVisitEnduserScheduleTemp] with(nolock)
		where ApmNo= @ApmNo;
		...
		if(@iCallSp=0)
		 begin
			Insert INTO [tblVisitEnduserSchedule] 
				...
				ApmTime, LimitTime,[status], 	visitType,	VisitorID,	AlertCellPhone,		AlertCellCarrier,TotalCharge,Relationship ,locationID, RecordOpt )
			SELECT roomID,apmNo,FacilityID , InmateID , InmateName , 	EndUserID,RequestedTime ,ApmDate ,1,GETDATE(), @ApprovedBy,
				ApmTime, LimitTime,@ScheduleStatus, 	visitType,	VisitorID,	AlertCellPhone,		AlertCellCarrier,@TotalCharge,Relationship,locationID  , @RecordOpt
