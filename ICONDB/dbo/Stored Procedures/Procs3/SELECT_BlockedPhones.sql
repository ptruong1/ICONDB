...
CREATE PROCEDURE [dbo].[SELECT_BlockedPhones]
(
	@FacilityID int
)
AS
...
IF (@FacilityID = 1 Or @FacilityID = 2)
	BEGIN
		SELECT TOP 100 PhoneNo, FacilityID, Username, A.ReasonID, T.Description as [Description], InputDate, R.Descript as [RequestReason]
...
ELSE	IF ( @FacilityID =74 or @FacilityID =75 or @FacilityID =76)
	BEGIN
		SELECT        PhoneNo, FacilityID, Username, A.ReasonID, T.Description as [Description], InputDate, R.Descript as [RequestReason]
		FROM            tblBlockedPhones A   with(nolock)  INNER JOIN tblBlockedReason T   with(nolock)  ON T.ReasonID = A.ReasonID
						INNER JOIN tblBlockedRequest R ON A.RequestID = R.RequestID
