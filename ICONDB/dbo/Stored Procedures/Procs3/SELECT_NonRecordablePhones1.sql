CREATE PROCEDURE [dbo].[SELECT_NonRecordablePhones1]
(
	@FacilityID int
)
AS
...
IF @FacilityID = 1
BEGIN
	SELECT TOP 100 PhoneNo, LastName, FirstName, T.Descript as [Description], A.DescriptID
...
BEGIN
	SELECT  PhoneNo, LastName, FirstName, T.Descript as [Description], A.DescriptID
	FROM    tblNonRecordPhones A   with(nolock)  INNER JOIN tblDescript T   with(nolock)  ON T.DescriptID = A.DescriptID
